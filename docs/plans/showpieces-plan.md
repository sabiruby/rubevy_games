...
* **（2026-09-17、著者判断）優先度をひっくり返した。** reflex はメインより **20 低い**
  優先度（`here.priority + 20`）になり、フレームの中で**最後に**聞く——後勝ちなので、それが舵になる。
  計画書が「メインより高く」と書いたのは「早いほど強い」という思い込みで、後勝ちとは噛み合っていなかった。
  スカウトの「舵を握り直す」回避策（`6.times { act; sleep 0.05 }`）は**消した**。`act` 1 回と
  `sleep 0.3` で足りる。`@swerve` は残る——被弾の瞬間に脳が飛ばしていた質問の答えで出る `act` は
  優先度では止められないので、脳が自分で手を引く取り決めがまだ要る。
  そのうえで、**脳が `@swerve` を見る場所をループの頭だけでなく `act` の直前にも置いた**。
  取り決めは「reflex が舵を握っているあいだ操作に触らない」なのだから、見るのは触る場所である。
  これは 2 番（往復 1 フレーム）を入れたあとに必要になった——被弾前に決めた `act` が
  被弾の 6 フレーム後ではなく 3 フレーム後に届くようになり、舵が入っている時間が半分になって
  しきい値（0.2 rad）に届かなくなったため。
  測った（`--headless 20` の selftest、`the heading changed within 0.3 s of the hit`）:

  | reflex | 脳の見る場所 | 質問の往復 | 通った回数 |
  |---|---|---|---|
  | `act` 1 回・低い優先度 | `act` の直前 | 1 フレーム | **10/10**（別の 3 回の 57 件すべてが 0.74 rad＝旋回速度の上限） |
  | `act` 1 回・低い優先度 | ループの頭だけ | 1 フレーム | 2/5 |
  | `act` 1 回・低い優先度 | ループの頭だけ | 2 フレーム | 5/5 |
  | `act` 1 回・高い優先度 | ループの頭だけ | 2 フレーム | 1/3 |
  | 握り直す・高い優先度（元の形） | ループの頭だけ | 2 フレーム | 3/3、ただし旋回が 0.25 rad まで落ちる |

  記録は `docs/worklog/2026-09-17-battle-followups.md`。
