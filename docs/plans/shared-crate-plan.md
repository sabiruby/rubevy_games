...
| S10 | 未着手（2026-09-21 著者「games_shell に 1 つ」「測って、案を出して止まる」）。**(1)** 「この機械のこの VM が 8 ms で買う命令数」45,600 の写し 2 つ（`garden/src/window.rs` と `sabibots/src/main.rs` の `INSTRUCTIONS_A_FRAME_BUYS`）を `games_shell::checks` に 1 つ — 既定値つきの設定（`Settings` の鍵で変えられる形。埋め込みの `const` にしない）、出どころ（S6 の実測の日と条件）は 1 か所、`scheduler_frames` / `handler_frames` の和の形（`2 + ceil(budget ÷ それ)`）も同じ場所に 1 つ。既定値は動かさない。6 通りの走行の行の一覧が前後で diff 空。**(2)** Battle の `script_budget`（rubevy の既定 200,000、出どころ不明）を箱庭の S5b-5 と同じ手順で測る: robot が最多の試合で命令数の分布（定常の中央値・99th・最大、最初のフレームの最大、持ち越し）を交互に 3 巡以上、`vmstat` で静かなことを見てから。**既定値は動かさず、案（数と導出、`handler_frames` が 7 から幾つになるか、変えると何を失うか）を報告して止まる。** 記録は `docs/worklog/2026-09-21-s10.md` と `docs/numbers.md` |
