...
| S11 | 未着手（2026-09-22、著者「最後まで続けて着手して」。F4 の前に、3 本に共通の小物を共有 crate と道具へ）。**(1) 走行の終わり方を 1 か所に**: factory を `checks_end_the_run()` へ乗り換え、`--shot` と判定の待ちを**両方向**（S9「判定が済んでも絵を待つ」、F2「絵を撮っても判定を待つ」— 今は factory の `take_shot` だけが後者を持つ）共有 crate へ上げて 3 本が同じものを使う。`done` の行の「走り続ける理由」は `checks_end_the_run()` の隣の 1 か所が言う（PC の `--shot` つきの走行が「a page has nothing to exit to」と言う嘘を直す。7 章の 09-21 S9（マージ）の行）。`--shot` つきの走行は `done` が 1 行増えることを `selftest-lines.md` に書く。**(2) 正でない設定を読み込み時に断る口を `games_shell::Settings` へ**: factory の `positive` / `counted`（F1）を上げ、S10 の `CheckPace` の `max(1.0)`、箱庭と Battle の同じ形の数（`.max(…)` で 0 を避けている設定の読み。grep で拾って一覧に）を同じ口へ。黙って切り上げず log で言って既定値を使う。既定値は動かさない。**(3) `web/build.sh` が `wasm-opt` を見つけられないとき黙って進まない**: PATH に無ければ既知の場所（`~/.local/binaryen-*/bin`）を探し、無ければ「縮めていない」を最後にもう一度はっきり言う（止めるかどうかは決めて理由を。CI は自分で入れているので CI の道は変えない）。**(4) 確認の作法を `docs/verification/selftest-lines.md` に**: 条件で待つのはフレーム数でも同じ（F3）、交互だけでなく向きも入れ替える（S10）、版が自分を名乗る行を見る・`docker/run.sh` の前に `docker/build.sh`（S10b）、`main()` の `warn!` は `LogPlugin` の前なので出ない（F3a）。`implementer.md` は著者の文書なので書き換えず、足す案を報告に。確認: 9 通りの走行（3 本 × ヘッドレス・窓・ブラウザ）が一覧と diff 空（(1) で `done` の行の文面が変わるならその分だけ）、`--shot` + 判定を 3 本とも窓で、設定に 0 や負を書いた走行で log に出て既定値で動くこと |
