...
| S9 | **済み**（2026-09-21、`s9` の 7 コミット `d79e676`…`5929c7c`）。6 項目すべて。**(6)** `games_shell::checks::checks_end_the_run()` — 判定が終わってよいかを「この台が終われるか」（`CHECKS_EXIT_WHEN_DONE`、そのまま残る）ではなく「この走行にまだ頼まれていることがあるか」で訊く。`--shot` があれば終わらない。`tools/fixedlines.sh` は **CR を先に落とす**（合成ログで実測: 前の版は 4 行全部が違うと言い、新しい版は diff 空。CR の無いログへの出力は 1 バイトも変わらない）。factory は F1 が別 worktree で動いているので `CHECKS_EXIT_WHEN_DONE` のまま。**(5)** Battle のハンドラの判定。**F0 の「1600×900 で毎回落ちる」は静かな機械では再現せず**（1600×900 × 3 と 1920×1080 × 1 で FAIL 0）、Chromium の CPU を **CDP で 20 倍絞って再現**した（1 FAIL、F0 と同じ形）。当たりのフレームとハンドラが走ったフレームを印字して測り、**静かなヘッドレス 145 件が全部 2 フレーム後**（= 構造どおり: publish は tick の後、次のフレームの tick で順番、判定は鎖と順序を持たないのでさらに次）、**絞ったページは 2・3・4**。窓は「0.3 秒 **または** `2 + ceil(script_budget ÷ 45,600)` フレーム」の長い方（出荷時 7）で、振れ幅の閾値も**その当たりがもらった窓**に従う（`HIT_WINDOW` の写しが 4 つ消えた）。つまみ `SABIBOTS_HANDLER_FRAMES` / `?selftest&handler_frames=N`。**5 走行目が穴を出した**: `P` が予算を 0 にするので上限が 2 に縮み、そもそも予算 0 ではハンドラが走れない → 予算は 1 度だけ読み、**窓が一時停止を跨いだ当たりは数えない**（4 つ目の除外）。直した後 CPU ×20 で 8 走行 FAIL 0（直す前は 1/1）。**(2)** egui の待ちが `scheduler_frames` から独立（`EGUI_FRAMES = 2`）。**導出**（離すのは 1 フレーム、取るのは 2 — `is_pointer_over_area` は前の pass の矩形を読むから）**で、実測も 3 走行とも 2 と 1**。待ちの舞台指示が「何を待ったか」を名乗るようになり（`waited N of M frame(s) for …`、verdict が無いので行の一覧は動かない）、それで初めて2 つの上限を測り分けられた。つまみ `GARDEN_EGUI_FRAMES`。**(4)** 箱庭の `Rubevy.ask("frame")` → `Rubevy.each_frame`。ゲーム側の `"frame"` の答えは**消した**（綴りを 2 つ残さない）。1 パスの命令数を**別ファイルに copy して `md5sum` で確かめ交互 6 巡**（前 9,802 / 後 8,994）測ったが、**群内のばらつきの方が大きく差とは言えない**——1 パスの費用は庭の混み具合で決まる。導出では数命令高いはずで、この測り方では解像できない。**(3)** §2.14 の 14 行を移した（箱庭 7 行 → `Light`（色 9 つ、`take_rgb` を足した）/ `Scenery` / `Furniture` / `Picture`、Battle 7 行 → `Look`。**z 座標一式は (a) のまま** — 重なりの**順序**が不変量で、設定にすると砂を機体の上に置ける。既定値は 1 つも動かさず、新しく決めた数 0。一覧は 285 → 300 件）。**(1)** Save を一時ディレクトリ／別の `localStorage` 鍵へ向けて押す段（`TheSave` は 4 つ目の `SystemParam`、`window_selftest` は 16 個ちょうどに。上限 `SAVE_FRAMES = 2` は導出）。**実物が穴を 1 つ出した**: ページの `forget` は鍵を空文字にするので、待ちは「読めること」ではなく「庭があること」でなければならなかった。箱庭の窓 45 → **47** 行・ブラウザ 46 → **48** 行。テスト 60 → 61、pageerror 0・requestfailed 0、unsafe 0。**窓の走行 2 通りは未実施**——`docker info` が segmentation fault で落ち、host に Vulkan の ICD が無い（`WGPU_BACKEND=gl` でも `Unable to find a GPU!`）。**`--shot` が絵を書くことは未確認**で、窓の一覧の 2 行はページの走行からの**推定**（その旨を `selftest-lines.md` に書いた）。**→ 同日、main へのマージのときに本体が Docker で回した: 窓 47 / 32 / 8 行とも一覧と diff 空・FAIL 0、`--shot` つきの箱庭は判定の後も走り続けて 30 秒で絵を書いた（推定は当たっていた。worklog §10）。**記録は `docs/worklog/2026-09-21-s9.md` |
| S10 | **(1) 済み／(2) 済み——著者が案 A**（2026-09-21、`s10` の 3 コミット、(1) が `fe64954`、測定が `bf84869`、既定値を入れた S10b が末尾）。**(1)** 45,600 の写し 2 つは **`games_shell::checks::CheckPace`** に 1 つ——既定値の名前は `INSTRUCTIONS_A_FRAME_BUYS`、鍵は 2 本で同じ **`checks_instructions_a_frame`**、出どころ（S6 と S5b-3 の 2 つの実測と、誤差の向きから遅い方を採る理由）は rustdoc 1 か所。和の形は `CheckPace::frames_to_wait(structural, budget)` で、**`structural` は引数**——S9 の worklog で確かめたとおり箱庭の 2（`Script` が着くフレーム + タスクが作られて走るフレーム）と Battle の 2（publish は tick の後 → 次のフレームの tick で順番 → 判定は鎖と順序を持たないのでさらに次）は**別の理由から出た別の 2** なので混ぜていない。「設定が効いている」の証拠に Battle が上限を名乗る舞台指示を 1 行足した（verdict が無いので行の一覧は動かない）: 既定 7 → `checks_instructions_a_frame=20000` で 12 → `script_budget=40000` も足して 4。既定値は動かさず、**6 通りの走行は `docs/verification/selftest-lines.md` と 6 つとも diff 空**（箱庭 13/47/48、Battle 4/32/33、FAIL 0、pageerror 0・requestfailed 0）、テスト 95 → 98、unsafe 0。**(2)** Battle の `script_budget` は**測っただけで動かしていない**。「上限の試合」をゲームが言わないので導いた（4 チーム × 9 台 = **36 台**。チーム数は `Match::TEAMS` と 4 組の絵、9 台は半径 20 の輪に差し渡し 3.2 の機体が 39 台しか入らないこと）。**16 走行 26,398 フレーム**: 中央値 3,872・99th 45,280・**最悪 67,749**、tick 中央値 0.23 ms・最悪 8.83 ms、**持ち越し 0・予算に届いたフレーム 0**（同梱の 4 台は最悪 2,730 = 既定の 1.4%）。箱庭と違い**起動フレームの山が無い**（試合が 1 台ずつ `spawn` を頼む）。機械を半分塞いでも命令数は静かな走行の幅の中（8 ms に当たっていない）。**1.74 は写さなかった**——著者が選んだ 45,000 から後ろ向きに出た比で、箱庭の最悪は決定的（3 走行とも同じ 23,912）、Battle の最悪はさいころ（46,438〜67,749）。**Battle 自身の理由からは係数を出せないと書き**、掛ける先を「16 走行の最悪」に変えた。**案 A = `1.74 × 65,704` → 114,000**（`handler_frames` 7 → **5**、出荷される余裕は 114,000 ÷ 67,749 = 1.68。`2 + ceil(118,000 ÷ 45,600)` も 5 なので、118,000 を選んでも判定は 1 フレームも変わらない）。案 B（余裕なしの 68,000）は次の走行のさいころが越えうるので採らない。案 C（据え置き、ただし「実測最悪の 3.0 倍・持ち越し 0」という出どころを書く）も並べた。設定ファイルで与えた走行で確かめた: 上限の試合を交互 3 巡で持ち越し 0・予算に届かず、判定は交互 **9 巡**（18 走行）4 行 FAIL 0・diff 空、決着の分布は見分けがつかず、ブラウザ（`localStorage`）も 33 行 FAIL 0（分布はページでは測っていない）。**暴走したスクリプトで予算が何を買うかも測った**: 200,000 なら 1 フレーム 212,600 命令・tick 7.91 ms、114,000 なら 122,600 命令・3.18 ms（60 Hz の 1 フレームの 47% → 19%）。**S10b（同日、著者が案 A を選んだので入れた）**: `script_budget` の既定は Battle 自身の `const SCRIPT_BUDGET = 114_000`（S5b-2 が `HEADLESS_SECONDS` などに与えたのと同じ「旗／店の既定」の形。鍵は今までどおり `script_budget`、`const` は既定値の名前としてだけ）。**予算は誰も言わなくても書くようになった**（条件を外した——言わなければ rubevy の 200,000 が残るため）。`frame_time` は条件のまま（8 ms は測っていない）。出どころは rustdoc と `docs/numbers.md` §4.5 に**正直に**: 36 台はゲームが言う上限ではない・1.74 は箱庭の比の借用で Battle 自身の理由からは係数を出せなかった・出荷される余裕は 1.68。`handler_frames` は**式のまま 7 → 5**（舞台指示の実物で 5 通りの走行で確認）。「出荷時 7」と書いてあった 10 か所を直した（過去の worklog は据え置き）。確認: 警告 0、テスト **101**（着手前と同数。`frames_to_wait(2, 200_000) == 7` は式のテストなので据え置き）、Battle ヘッドレス ×3（4 行 FAIL 0。1 走行だけ「落ちた機体」の行が `--`——`script_budget=200000` の対照 3 走行でも 3 回中 2 回出るさいころで、`selftest-lines.md` が「verdict が動く 1 行」と書いている行）、窓 32 行・ブラウザ 33 行とも diff 空・FAIL 0・pageerror 0・requestfailed 0、箱庭ヘッドレス 13 行 diff 空、前の版の `localStorage`（`script_budget=200000`）のブラウザは**その値が尊重されて上限 7**・33 行 FAIL 0。記録は `docs/worklog/2026-09-21-s10.md` §5、`docs/numbers.md` §1.8 と §4.5 |
| S11 | **済み**（2026-09-22、`s11` の 4 コミット: (1) `03b3594`、(2) `2c1e783`、(3) `1857deb`、(4) と記録が最後の 1 つ）。4 項目すべて。**(1)** 走行が頼まれることは 2 つ（判定と絵）で、**どちらも済んだ瞬間に自分で `AppExit` を書いていた**ので互いを打ち切れた（S9 が片方向を `argv` で、F1・F2 がもう片方向を factory の中だけで直していた — 同じ 1 つの問いの半分ずつが 2 か所に別々の形で）。**`games_shell::checks::Errands`** が「何を頼まれ・何がまだか・なぜ走り続けるか」を全部答える。**`argv` ではなく Resource にした**: 半分（絵を撮ったか）は command line の事実ではない／ページには command line が無い／毎フレーム読む system が読む。`Resource` 兼 `Plugin` で、ゲームが書くのは `add_plugins(Errands::of("the garden", checks, shot.is_some()))` の 1 行（2 行に割ると片方の書き忘れが「何も頼まれていない走行」に見える）。`AppExit` を書くのも `done` の行を書くのも `end_the_run` の 1 か所で、ゲームが渡すのは主語 1 語。**PC の `--shot` つきの走行が言っていた嘘が直った**: `(a page has nothing to exit to)` → **`(the picture is still to be taken)`**。**「何も頼まれていない走行は終わらない」**（空の一覧は済んだ一覧ではない）を単体テストで固定。3 本の `take_shot` は絵を撮るだけになり、`MessageWriter<AppExit>` が `ResMut<Errands>` に替わった（箱庭と Battle の判定は Bevy の引数 16 個ちょうどなので**足すのではなく入れ替え**）。factory の `?stress` の終わりも同じ口に。**(2)** 読みの行に書いてあった `.max(0.001)` / `.max(1.0)` / `.max(3.0)` **20 か所**を `Settings::positive(key, default)` と **`counted(key, least, default)`** へ。`least` を引数にしたのは床の理由が呼ぶ側にしかないから（木 0・影 1・空のドーム 3）。`&self` + `Mutex` にしたのは、パネルに渡すクロージャ（S5b-1）と 3 本の `read_from(&Settings)` を全部可変にするのは狭いことを言うために広く触るから。`number` は**文字も断る**。断りは溜めて **`SettingsRefusalsPlugin` が `Startup` で言う**（`main()` の `warn!` は `LogPlugin` の前なので出ない — F3a）。**既定値は 1 つも動いていない**（`sky_sides=1` は三角形ではなく既定の 12 になり、log に理由が出る）。**(3)** `web/build.sh` が `~/.local/binaryen-*/bin/wasm-opt` を探し、見つからなければ**最後にもう一度**はっきり言う。**止めない**（縮んでいなくても動くし、Playwright の前の 1 本はそれを気にしない走行。大きさが効く走行 = CI は自分で binaryen を入れるのでこの道を通らない）。CI の道は 1 文字も変えていない。**(4)** `selftest-lines.md` に「What a check has to get right」の節（条件で待つのはフレーム数でも同じ F3／交互だけでなく向きも入れ替える S10／版が自分を名乗る行を見て `docker/build.sh` を先に S10b／`main()` の `warn!` は出ない F3a）。`implementer.md` への案は報告に文面で。確認: 警告 0、テスト **110 → 114**、足した行に unsafe 0、wasm の `cargo check` 3 本、**9 通りの走行は `selftest-lines.md` と 9 つとも diff 空**（箱庭 13/47/48、Battle 4/32/33、factory 19/22/23、FAIL 0、pageerror 0・requestfailed 0）、**`--shot` + 判定を 3 本とも窓で**（絵が出て、判定の行が全部出て、`done` が PC の理由を言う。factory は既定の 8 秒だと絵が先に済むので `done` の行が出ない — それも記録した）、**設定に 0・負・文字を書いた走行 3 本**で断りが log に出て既定値で走る。記録は `docs/worklog/2026-09-22-s11.md` |
...
| **F7 が足したもの**（Factory の段階だが**共有 crate に入った**。2026-09-22、`f7` の 5 コミット） | **(1) `Guide` が 2 つの窓になった**: 本文（`H`、今までどおり）と**キーの表の小窓**（`Guide::keys_window`、角に立つ、動かせる、**3 本とも既定でオン** — 著者の決定）。表は**同時に 2 か所には出ない**（小窓が出ている間、本文は「小窓に出ています」の 1 行）。書く口は 1 つ（**`Guide::show_keys_window(on, settings)`** — `guide_keys` を `pub` にした F5 と同じ理由で公開。本文のトグル・小窓の ✕・判定が同じ道を通り、`guide_keys_window` として store に残る）。`GuideStyle` に鍵 2 つ（`guide_keys_window` / `guide_keys_margin`）。**小窓の大きさは数を持たない**（egui が中身に合わせる）。**`Order::Foreground`** — 箱庭と Battle はエディタを起動時に開くので、最初の絵では小窓の見出しがその下に隠れていた。**(2) カメラにズームのキー**: `CameraKeys::zoom_in` / `zoom_out`（`=`/`+` と numpad、`-`）、`CameraControls::notches_per_key`（既定 1.0 = ホイールの 1 ノッチ、鍵 `camera_notches_per_key`）。**限界も刻みもホイールと同じ `zoom_by` を通る**。足したのは口が 3 つ: `zoom_the_view`（中心は画面の真ん中）・**`zoom_about`（ホイール。カーソルの下の点をカーソルの下に残す）**・`notches_to`（「この見え方まで」をノッチ数で言う口。ボタンが `CameraView` に直接書いて限界を踏み外すのを防ぐ）。**箱庭と Battle には (2) は届かない** — `CameraPlugin` を使っているのは Factory だけ（箱庭は自前の 3D オービット、Battle は `ArenaPlugin`）。**箱庭と Battle の 9 通りのうち 4 通り（窓とブラウザ）を走らせ、行の一覧は 4 つとも diff 空**。記録は `docs/worklog/2026-09-22-factory-F7.md` |
...
|---|---|---|---|---|
| 09-22 F7 | **`CameraPlugin` の利用者は 1 本だけ**（Factory）。計画 §6 の F7 は「`CameraControls` に足すので箱庭と Battle にも `+`/`-` が届く」と書いていたが、箱庭は自前の 3D オービット（`Eye` / `Orbit`）、Battle は `ArenaPlugin`（ズームが無い）なので**届かない**。箱庭のホイールにも同じ「トラックパッドでは気づけない」問題があり、直すなら箱庭自身の camera に 2 行 | `garden/src/main.rs` の `Eye`、`games-shell/src/camera.rs` | 箱庭／共有 crate | **本体が原則から決めた（F7、09-22）**: **箱庭には足さない**。F7 は著者が Factory について言った 6 点への答えで、箱庭の camera を触ると 3 本目の段階が 1 本目の遊びを変える。ガイドの 1 行と数の出どころが要るので、やるなら箱庭自身の段として |
| 09-22 F7 | **egui の `EguiWantsInput` は偽装できない**（フィールドが private で setter が無い）ので、「マウスがパネルの上にあるとき何かが起きない」は**マウスを動かして確かめるしかない**。動かす口は 2 つ要る: `Window::set_cursor_position`（ゲームが読む）と `WindowEvent::CursorMoved`（bevy_egui が egui のポインタを作る元）。片方だけだと 2 つが食い違う | `factory/src/main.rs` の `point_the_mouse` | 判定の作法／共有 crate の候補 | 未決。3 本目で要ったので、4 本目が要るなら `games_shell::checks` に上げる |
| 09-22 F7 | **コンテナの走行では本物のポインタがどこにあるか決まらない**（ある走行は窓の外 = `cursor_position()` が `None`、別の走行は 77, 9 = HUD の上）。マウスの位置に依る判定は、偽装しないかぎり**さいころ**になる | `docker/run.sh` の走行 | 判定の作法／本の素材 | 未決 |
| 09-22 F7 | **整数倍ズームは 1 ノッチを飲み込んでいた**（1/10 のノッチに対し 3:1 → 4:1 は 1/3）。**F1 から公開版まで、ホイールを 1 ノッチずつ回しても何も起きなかった** — 速く回すと 1 フレームに複数ノッチ届くので誰も気づかなかった。判定が `+` を 1 回押して見つけた | `factory/src/draw.rs` の `snap_zoom` | Factory（直した）／本の素材 | **F7 で直した**（丸めが打ち消すときは整数倍を 1 段動かす）。**本の素材**: 「丸めと刻みが同じ桁にあると、小さい入力が黙って消える」 |
...
| 09-21 S9 | **`INSTRUCTIONS_A_FRAME_BUYS` 45,600 の写しが 2 つになった**（`garden/src/window.rs` と `sabibots/src/main.rs`）。偶然同じ数ではなく**同じ 1 つの事実**（この機械のこの VM が 8 ms で買う命令数）なので、本来は両方が届く crate に 1 つあるべき。2 つのバイナリにまたがるので cite すらできない。同じ段階で **Battle の `script_budget` 200,000（出どころ不明）が判定の上限の割り算に入った** | `garden/src/window.rs`、`sabibots/src/main.rs` | 共有 crate（構成）／出どころの無い数 | **両方 S10 で済んだ**: 45,600 は `games_shell::checks::CheckPace` に 1 つ（設定、鍵は 2 本で同じ）。Battle の予算は測った（上限の試合 36 台、16 走行 26,398 フレーム、最悪 67,749、持ち越し 0）——案 114,000 を出して止まり、**S10b で著者が案 A を選んで既定が 200,000 → 114,000 になった**（`SCRIPT_BUDGET`。`handler_frames` は式のまま 7 → 5） |
| 09-21 S10 | **予算は「天井」であって「床」ではない**: どの `script_budget` でも 1 フレームの命令数はそれを **6〜52% 越える**（41,000 → 62,500、114,000 → 122,600、200,000 → 212,600。103,000 と 114,000 は同じ 122,600 になる）。判定の上限を予算の割り算で出している以上、知っておく値 | rubevy の `tick_scripts` / `task_run_limits`、測定は `docs/worklog/2026-09-21-s10.md` §2.9 | rubevy（API と文書） | 未決。rubevy の計画 7 章に写す候補 |
| 09-21 S10 | **Battle には機体数の上限がどこにも無い**（`team :red, robots: %w[…]` は何台でも書ける）。「上限の試合」36 台は担当が輪の幾何から導いた数で、**ゲームが言った数ではない**。箱庭が `world.rb` の `pop_max` を言うのと対照的で、**上限を言わないゲームでは「測って決めた既定値」がいつでも破られうる** | `sabibots/ruby/match_prelude.rb`、`matches/training.rb` | Battle（設計）／本の素材 | 未決 |
| 09-21 S10 | **`Match#start` の角度に「1 チーム 2 台」が直に書いてある**（`(t * 2 + i) * 6.28 / (teams.size * robots.size)` の `t * 2`）。2 台以外では機体が輪の一部に固まって出て `separate_robots` が押し広げる。`docs/numbers.md` にも無い | `sabibots/ruby/match_prelude.rb` | Battle（一覧の抜け／DSL の作り） | 未決 |
| 09-21 S10 | **交互に回すだけでは足りず、向きも入れ替える必要があった**: 交互 6 巡で「予算を下げると当たりが増える」と読める形（16.7 対 22.7）が出て、順番を逆にした 3 巡で消えた——差は「対の何番目に走ったか」に付いていた（17.5 対 21.1）。**巡を足しても向きが同じなら同じ偏りを買い足すだけ** | 計測の作法 | 作法（`implementer.md`）／本の素材 | 未決。`implementer.md` に 1 行足す候補 |
| 09-21 S10 | **`--headless N` の `N` は「試合の長さの上限」であって走行の長さではない**（`stop_when_over` は勝者が出たらそこで終える）。`--headless 180` の走行が 18.7 秒で終わるので、分布を取るときは「何フレーム取れたか」を必ず書く | `sabibots/src/main.rs` の `stop_when_over` | 測り方／Battle | 未決 |
| 09-21 S10 | **箱庭と Battle で「最悪フレーム」の性格が違う**: 箱庭のそれは決定的な起動フレーム（3 走行とも 1 命令まで同じ）、Battle のそれはさいころ（16 走行で 46,438〜67,749）。**同じ「測って 1.74 倍」という手続きが、2 つのゲームで別の強さの保証になる** | `docs/worklog/2026-09-21-s10.md` §2.6 | 本の素材（数の決め方） | 未決 |
| 09-22 S11 | **箱庭のホイールの判定は「エディタが窓より狭い」ことに寄りかかっている**: `editor_width=1400` を viewport 1280 のページに与えると狙う点がパネルの下に落ち、`egui holds the pointer: true` で FAIL する（2 回とも同じ 1 行）。**設定が効いた結果の FAIL** であって、判定したかったことは測れていない — 正しくは `--`。S5b-1 が 1400 を試したのは 1600×900 の窓なので当たらなかった | `garden/src/window.rs` の `FakePointer` の狙う点 | 箱庭の判定の作り | 未決（S11 は直していない。`docs/worklog/2026-09-22-s11.md` §4.6）|
| 09-22 S11 | **`take_shot` は 3 本とも同じ 20 行で、違うのは猶予の秒だけ**（箱庭 2.0、Battle と factory 1.0）。その秒の出どころも、3 本で違う理由も、どこにも書かれていない。S11 は「終わりを決める所」だけを上げたので、絵を撮る system は 3 か所のまま | 3 本の `take_shot` | 共有 crate（構成）／出どころの無い数 | 未決 |
| 09-22 S11 | **`rubevy-egui` の 4 つの床には (2) の口が届かない**（`vm_regs_frames`・`vm_value_chars`・`code_rows`・`code_chars`）。この crate は設計上 `Settings` を知らず、`impl Fn(&str) -> Option<f32>` を受け取る。直すには公開の署名を変えることになる | `crates/rubevy-egui/src/{inspect,code}.rs` | 共有 crate（API）／**公開に関わる** | 著者判断待ち |
| 09-22 S11 | **`SettingsRefusalsPlugin` はゲームが足さないと何も言わない**（断りは溜まるが誰も読まない、という静かな失敗）。`games_shell::remembered` は `App` を触らない関数なので畳み込めなかった | `games-shell/src/settings.rs` | 共有 crate（罠）| 未決 |
| 09-21 S9 | **「向きが変わった」は弱い判定**: scout は `run` のループでも曲がるので、ハンドラが走らなかった走行でも `ok` になりうる（直す前の FAIL 1 件がその形だった）。2 行のうち本体は「ハンドラが走った」の方 | `sabibots/src/main.rs` の `handler_selftest` | Battle の判定の作り | 未決 |
| 09-21 S9 | **Battle に一覧にも無い直書きの数が残っている**: 弾のスプライトの縦横比 `× 0.55` / `× 1.3`、爆発の `× 0.4`、砲身の付け根のずらし 1.0、床が壁の外へはみ出す幅。**網は 2 回掛けても漏れる**（S5a は `const` に寄り、S5b-5 は「関数の本体の数」を拾ったのに変数に掛ける係数が漏れた） | `sabibots/src/main.rs` | 一覧の作り | 未決 |
| 09-21 S9 | **`window_selftest` が Bevy の引数ちょうど 16 個になった**（`TheSave` が 4 つ目の `SystemParam`）。次に足す人はまた 1 つまとめる | `garden/src/window.rs` | 箱庭の判定の作り | 未決 |
| 09-21 S9 | **本の素材**: 「別々の理由の待ちに 1 つの数を使うと、片方の理由が動いたとき、もう片方が黙って壊れる」の続き——**待ちが自分の名前を名乗るまで、どの待ちが遅かったのかは測れない**。S9 が egui の 2 フレームを測れたのは舞台指示に「何を待ったか」を書いた 1 行のおかげ | `garden/src/window.rs` の `Turn::what` | 本の素材 | book の findings に写す |
| 09-21 S5b-5 | **egui の待ちが VM の数に相乗りしている**: `Turn::EguiHasThePointer` の上限が `scheduler_frames`（予算から計算）で、予算が 1/5 になった結果 7 → 3 フレームになり、egui の待ちが先に足りなくなった（8 本同時 88 走行中 3 が諦めた）。今は説明パネルを閉じたので当たらないが、`script_budget` を下げればまた当たる。S7 の「3 つの待ちのうち最大の 1 つの数でよい」という決定の帰結 — **別々の理由の待ちを 1 つの数で賄うと、片方の理由が動いたとき、もう片方が黙って壊れる** | `garden/src/window.rs` の `Turn` | 箱庭の判定／本の素材 | **本体が原則から決めた: 待ちごとに自分の上限を持たせる**（egui の上限は egui の理由から出す — 測って）。**S9 で入った**（`EGUI_FRAMES = 2`。導出でも実測でも 2。待ちは 3 つの上限を持つ: VM の `scheduler_frames`、egui の 2、Save の 2）。book の findings に写す |
...
| 09-21 S9（マージ） | PC の `--shot` つきの走行の `done` の行が「a page has nothing to exit to」と言う（ブラウザの理由のまま。PC で走り続ける理由は「絵がまだ」） | 3 本の selftest の `done` の行、`games_shell::checks` | 共有 crate | **本体が原則から決めた: 理由は `checks_end_the_run()` の隣で 1 か所が言う**（走り続ける理由を返す形にして 3 本が同じ文を出す）。factory が `checks_end_the_run()` に乗り換える段（F2 の後）と一緒に |
