...
| S8 | 実行中。**調査のみ**: ブラウザで `2 s paused: every creature is where it was` / `nobody got hungrier` が落ちた件（下の 7 章） |
...
| S5b | **5 つに分けて順に**（同じファイルを広く触るので並行させない。1 つずつレビューして main に入れる）: **S5b-1** 共有 crate 2 つの数（**済み**、2026-09-21、`3161f75`・`0a6b961`・`c2e7424`、main に取り込み済み: `EditorLayout`（Resource）・`EditorColors`・`InspectStyle`・`VmClock::smoothing`・`CodeStyle`・`HudStyle`・`GuideStyle`・`ArenaPlugin::floor_margin`、`Settings` の鍵 27 個を `PanelSettingsPlugin` が読む。`rubevy-egui` は `Settings` を知らず、`read_from(|key| …)` のクロージャで受ける。`ArenaSize` は既定を持たず Battle が `ARENA_HALF_WIDTH = 32.0` を渡す。既定値は 1 つも動かさず、新しく決めた数は 0。一覧は 33 → 42 件（S3 のカメラ 8 件ほかを足した）、全体 269 件・不明 130。6 通りの走行の行の集合は前後で diff 空、`garden.settings.txt` に `editor_width=1400` と `guide_height=100` を置いた窓の走行も 44 行 FAIL 0。テスト 34 → 46、wasm +0.12% / +0.20%）→ **S5b-2** Battle（実行中。Rust と Ruby の二重の数を 1 か所に、名前の無い 100 と 3000、(b) と (c)）→ **S5b-3** 箱庭の動かす側の数 (c)（creature の VM の予算と `frame_time` をゲームの設定に — 上限の庭で測る。`SCHEDULER_FRAMES` を設定から計算）→ **S5b-4** 箱庭の遊びの数 (b) を Ruby 側へ（変異率の判定は VM から読む、種ごとに 1 つの `Handle`、`TOUCH_REACH`、芽の最小間隔）→ **S5b-5** 判定の側 (d)（`--` と `n/a` を 1 つに、残りの 0.6 秒、ホイールの `held`、`docker/run.sh` が `<GAME>_` の環境変数を全部渡す）。**著者判断済み（2026-09-20）: 推奨どおり進める** — 分類案のとおり（遊びの数は Ruby 側、動かす側は `Settings`・引数、判定の閾値は検査側）。出どころ不明の 125 件は「不明」と書いたまま移し、**既定値は 1 つも動かさない**。変異率の判定は VM から実際の値を読む形に直す。Battle の Rust / Ruby の二重の数は 1 か所に。分類に迷った 10 件は本体の案で進めて結果を報告。selftest が前後で同じことが条件 |
