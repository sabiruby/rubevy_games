...
| S6 | 実行中。**箱庭の窓のチェックの揺れの調査**（コードは変えない）: 無改変で 3 走行中 2 走行が FAIL した 2 件（`Revert puts every beetle back on the file`、`every restarted beetle's new task has run`）と、S3 で出た 3 件目（`Apply restarts every beetle on the edited text`）を、PC の窓と**ブラウザ**の両方で何十回か回して数え、原因を割り、直し方を並べて報告して止まる |
