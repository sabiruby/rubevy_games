...
| S5b | **5 つに分けて順に**（同じファイルを広く触るので並行させない。1 つずつレビューして main に入れる）: **S5b-1** 共有 crate 2 つの数（実行中）→ **S5b-2** Battle（Rust と Ruby の二重の数を 1 か所に、名前の無い 100 と 3000、(b) と (c)）→ **S5b-3** 箱庭の動かす側の数 (c)（creature の VM の予算と `frame_time` をゲームの設定に — 上限の庭で測る。`SCHEDULER_FRAMES` を設定から計算）→ **S5b-4** 箱庭の遊びの数 (b) を Ruby 側へ（変異率の判定は VM から読む、種ごとに 1 つの `Handle`、`TOUCH_REACH`、芽の最小間隔）→ **S5b-5** 判定の側 (d)（`--` と `n/a` を 1 つに、残りの 0.6 秒、ホイールの `held`、`docker/run.sh` が `<GAME>_` の環境変数を全部渡す）。**著者判断済み（2026-09-20）: 推奨どおり進める** — 分類案のとおり（遊びの数は Ruby 側、動かす側は `Settings`・引数、判定の閾値は検査側）。出どころ不明の 125 件は「不明」と書いたまま移し、**既定値は 1 つも動かさない**。変異率の判定は VM から実際の値を読む形に直す。Battle の Rust / Ruby の二重の数は 1 か所に。分類に迷った 10 件は本体の案で進めて結果を報告。selftest が前後で同じことが条件 |
