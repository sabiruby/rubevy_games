...
| W3 | **済み**（`d7ef4b9` `b50ab46` `24c1715`、2026-09-17）。`F3` で `world.rb` をエディタに（`Brains` 3 枠目、Ctrl+Enter / Ctrl+S / Revert / `Watch`）、HUD に世界 VM の命令数、窓のセルフテスト +11 行（40 行全通過、WSLg + lavapipe で実行）、wasm は `build.rs` が既に拾っていて無変更で通る、ガイドに段落 1 つと `F3` の行（英日、不足 5 文字を切り直し）、`docs/garden.md` に世界 VM の節。**予算の測り直し**: 上限の箱庭（90 株 24 匹、392 フレーム）で 1 パス中央 24,670・最大 25,837、tick 中央 2.65 ms・最大 4.86 ms。W1 の 1 次式は W2 の規則（つがい探し）では成り立たない（残差 5.2%）ので外挿をやめ、実測で `budget = 45_000`（最大の 1.74 倍）を据え置き。命令数が `frame_time` より先に噛む（45,000 ≈ 4.8 ms）。**直したバグ**: `reload_changed` が `world_prelude.rb` を `prelude.rb` と取り違え、保存のたびに全生き物を再起動していた。**未着手（範囲外）**: 世界の VM パネル（F2 は生き物のみ）、世界のエディタの行の帯、コンパイルエラーの行番号の prelude 分のずれ（生き物側も同じ）、`editor.noun` の hover の文 |
