...
| G10 | 同期読み（rubevy `docs/plans/sync-access-plan.md` の S3、2026-09-17）: 読みがフレームを食わなくなったので、HUD の「1 判断あたりのフレーム数」を「1 判断あたりの命令数」に置き換え、VM パネルの読み待ちの意味を「予算切れで残った読み」に、規則の chain を `.before(RubevySet::Tick)` に。両ゲーム（Battle はコンポーネントを読まないので文面だけ） | 済み（`5a726d7` 計測とパネル・`64b73cd` 順序、およびその次のコミットのパネルの枝）。**6 つ目の判定（ビートルの反射）は 10 回中 4 回落ちる状態**で、原因は `flee_from` の向きではなく (A) 測っている 0.5 秒の窓に 2 通目の `"touched"` が来ること (B) ハンドラの `sleep 0.5` と判定の 0.5 秒後が同じ数であること——どちらも判定かハンドラの作り直しで、**著者判断待ち**（`docs/worklog/2026-09-17-sync-reads.md` §8） |
