...
| G10 | 同期読み（rubevy `docs/plans/sync-access-plan.md` の S3、2026-09-17）: 読みがフレームを食わなくなったので、HUD の「1 判断あたりのフレーム数」を「1 判断あたりの命令数」に置き換え、VM パネルの「コンポーネント読み待ち」を消す。両ゲーム（Battle はコンポーネントを読まないので文面だけ） | 済み（`5a726d7` 計測とパネル）。**規則の chain を `.before(RubevySet::Tick)` に置く件は着手前に戻した**（6 つ目の判定が半分の回で落ちる。`docs/worklog/2026-09-17-sync-reads.md` §2、**著者判断待ち**） |
