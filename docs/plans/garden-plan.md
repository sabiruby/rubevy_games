...
| G7 | 名前の整理（著者判断 2026-09-17）: DSL `reflex` → `on`、説明文の「頭脳」→「行動アルゴリズム」、英語の brain/mind → behaviour、キー表は短縮形。両ゲーム | 済み（`00c0044` 改名・`5af3bcd` 文面・`7d868ea` フォントと絵） |
| G8 | 見た目（著者の 3 回目の試遊 2026-09-17）: フィールドの外が単色で浮いている → 地平線、動物と虫の色が似ている → 色分けとモデル候補 | 済み（`23a68db` 地平線・`84397ab` 種の色・`f0ecdef` 甲虫 3 案）。甲虫のモデルは**著者判断待ち** |
| G9 | 一時停止と VM パネル（著者判断 2026-09-17）: `P` で世界ごと止める、VM パネルは既定で閉じ上半分を人向けに。両ゲーム | 済み（`7c0be27` 世界ごと停止・`d00e890` パネル・`3cce28e` 文面とフォントと絵） |
| G10 | 同期読み（rubevy `docs/plans/sync-access-plan.md` の S3、2026-09-17）: 読みがフレームを食わなくなったので、HUD の「1 判断あたりのフレーム数」を「1 判断あたりの命令数」に置き換え、VM パネルの読み待ちの意味を「予算切れで残った読み」に、規則の chain を `.before(RubevySet::Tick)` に、6 つ目の判定の測り方を仕様（「0.5 秒**以内に**向きが変わった」）に合わせて 0.5 秒の**窓**で見る形に。両ゲーム（Battle はコンポーネントを読まないので文面だけ） | 済み（`5a726d7` 計測とパネル・`bff4003` パネルの枝・`64b73cd` 順序・そのあとのコミットで判定の窓化）。**6 つ目は 10 回中 10 回 ok、242 接触で外れ 0**（窓にする前は 248 接触で外れ 7・4 回 FAIL）。閾値は 1 つも動かしていない。**残り: 5 つ目（草に着く）が順序ありの 26 回で 3 回落ちる**（順序なしは 24 回で 0 回。p ≈ 0.24 なので「順序のせい」とまでは言えない）。4 つ目（すり抜け）は順序なし 1/24・順序あり 1/26 で**元から時々落ちる**。どちらも触っていない、**著者判断待ち**（`docs/worklog/2026-09-17-sync-reads.md` §8・§9） || W1 | **世界の規則を Ruby に**（別計画書 `docs/plans/garden-world-plan.md` 第 2 版、2026-09-17）: 2 本目の VM（`struct World`）に `ruby/world.rb` を 1 本走らせ、草・空腹・食事・餓死を Rust の 5 システムから引っ越す。`garden.within` は `answer_in_tick` で同じ tick に答える | 済み（`87038d9` `bd8fef7` `113f7d5` `39c60f1`）。12 判定 ×5 全通過。予算は測って `45_000`（`docs/worklog/2026-09-17-garden-world.md` §4） |
| W2 | つがい・子を Ruby に（`court` / `hatch` 削除、`Breeding` を Reflect、`Creature.parent` 追加）、`every` / `tell` / 季節 / `on(:season)` / 13 番目の判定 | 済み（`ec3fd46` `b3b3137` `6f37d68`）。13 判定 ×10 で 128 ok / 2 FAIL（5 番・7 番の既知の揺れ。閾値は動かしていない） |
| W3 | 窓とブラウザと文書: `F3` で `world.rb` をエディタに（`Brains` の 3 枠目・Ctrl+Enter・Ctrl+S・`Watch`）、HUD に世界 VM の命令数/フレーム、窓のセルフテストに 11 行、`docs/garden.md` の規則の表と世界 VM の節、**予算の測り直し** | 済み（`docs/worklog/2026-09-17-garden-world.md` §14〜）。13 判定 ×5 全通過、窓 40 行すべて ok（docker + lavapipe）、`web/build.sh garden` 通る。予算 45,000 は据え置き、**根拠を外挿から上限での実測に差し替え**（上限 90 株 24 匹で最大 25,837 命令・tick 4.86 ms） |
...
**実装で分かったこと**（`docs/worklog/2026-09-17-garden-G7.md`）:

* **`brain` と `reflex` は範囲が違う。** `reflex` は識別子まで全部（計画書 1 項の指示どおり）。
  `brain`/`mind` は 2 項が列挙した場所 — つまり**人が読む文字列**だけで、Rust の `Mind`、`Brains`、
  `give_mind`、`watch_minds`、`brain: Option<String>` とそれを名指しするコメントはそのまま。
  境目は「画面に出るか」で引いた。`prelude.rb` はエディタに出ない（エディタが開くのは種のファイル）ので
  残り、`beetle.rb` / `scout.rb` のコメントは**出るので**直した。`grep` に残るものは報告に一覧がある。
* **語の長さが変わるので、罫線と表は機械では直らない。** `# --- the brain ---` の `-` を 4 つ、
  hover の `reflex(es)` を `handler(s)` に、`a reflex(:ate)` の冠詞を `an` に、手で直した。
  `` `reflex`es `` のような**識別子を英語の複数形にしている散文**も機械では壊れる。
* **worklog のファイル名は歴史なので置換しない。** `prelude.rb` と `docs/garden.md` が参照している
  `docs/worklog/2026-09-16-reflex.md` が `-handler.md` になっていた。`grep -rn reflex` に残る 1 件
  （`docs/README.md` の目次行）も、**ファイル名が `reflex` である以上、目次が嘘をつかないための橋**として残した。
  英文に 1 か所まで許された "reflex" は使っていない。
* **フォントは初めて小さくなった。** 337 → 334 文字、66,796 → 65,904 バイト。
  入ったのは `ゴベ処理` の 4 文字（「行動アル**ゴ**リズム」「イ**ベ**ント」「**処理**」）だけで、
  「頭脳」「反応」「専用」「考え」「もう」が消えて 7 文字落ちた。
* **文面を変えると絵が古くなる。差し替えは 4 枚ではなく 7 枚だった。** `--shot` はエディタを開いた状態で撮るので、
  `docs/garden.png`・`docs/garden-night.png`・`docs/vm-inspector.png` に `reflex(:night)` / `reflex(:hit)` が写っていた。
  フォントはスクリプトが検出できるが、絵は**1 枚ずつ開いて読む**しかない。
  `docs/vm-inspector.png` は 8 秒だとレジスタが全部 `nil` で隣の段落と食い違うので 14 秒で撮り直した。

## 見た目（G8）

著者の指摘（2026-09-17）: 「フィールドの外側が単色で浮いている」「動物と虫の色が似ていて差が分かりにくい」。負荷とリソースを増やさずに直す。

1. **地平線**: フィールドの外に同じ地面色の大きな板（三角形 2 枚）を敷き、カメラに `DistanceFog` を付けて遠くを空の色に溶かす。空は `ClearColor` の単色ではなく、
   **大きな球の内側に頂点色のグラデーション**（テクスチャ無し、三角形数十枚）。フォグの色と空の色は昼夜で `day_night` が一緒に動かす（夜は月明かりの青）。
...
### 実装で分かったこと（2026-09-17、`docs/worklog/2026-09-17-garden-G8.md`）

* **既定のカメラでは空が 1 ピクセルも映らない。** 距離 42・ピッチ 0.85 rad = **49° の見下ろし**、垂直画角 45° なので
  フレームの上端は仰角 −26°、**真の地平線はフレームの 26° 上**。板を大きくしただけだと画面が一面の緑になる
  （前に青かった場所が緑になっただけ）。この角度で使える奥行きは**フィールドの遠い角 57 からフレーム上端 72 までの 15 単位**しか無く、
  「遠くを溶かす」ために置いた最初の霧（`start = 距離 + 25`、深さ 160）は**フレームに 1 ピクセルも入っていなかった**。
  `FOG_NEAR = 14` / `FOG_DEPTH = 45` で既定の絵に霞の帯が出る。既定のピッチは計画に無いので変えていない。
* **空と遠景の地面はカメラに乗せた。** 球は半径 500 をカメラ位置に、板は XZ だけ追わせる。
  原点に固定して最悪ケース（原点から 138）に耐える大きさにするより小さく済み、**カメラの既定の far 面（1,000）で足りるので投影に触っていない**。
  縁が常に同じ距離なので「霧が飽和し切る距離 < 板の縁」が保証できる。
* **頂点色は掛けられるのではなく代入される。** `bevy_pbr` の `pbr_fragment.wgsl`:
  `#ifdef VERTEX_COLORS  pbr_input.material.base_color = in.color;`。なので空は 49 個の `[f32; 4]` で済み、
  シェーダもテクスチャもキューブマップも要らない（12 辺 × 4 輪 ＝ **84 三角形**）。`unlit` + `cull_mode: None` で
  巻き方向と法線を考えずに済み、`fog_enabled: false` が無いと 500 単位先の球が霧の色 1 色で塗り潰される。
* **`colormap.png` は陰影地図ではなく平らな色見本だった。** 512 × 512 に `ff7e44` `cf534f` … の四角が並んでいるだけ。
  計画の「掛け色でテクスチャの濃淡は残る」は成り立たず、橙のカニに青緑を掛けると暗い茶色になる。
  クローンで `base_color_texture: None` にした（失うのは目、残るのは造形）。**著者が目を戻したいなら 1 行で戻せる。**
* **外周の木はコンポーネントを 1 つも持たせなかった。** `Tree` を付けると `garden.nearest(:Tree)` が 7 本ではなく 23 本を意味し始め、
  しかも壁の外で**絶対に届かない木**を教えることになる。サイコロも `Dice` リソースとは別にした
  （窓の有無で庭の配置が変わるため）。
* **`--eye UNITS` を足した（計画外）。** 甲虫 3 案の絵が既定の距離 42 では虫が 30 ピクセルで、区別が付かない。18 で 70 ピクセル。
* **同じ構図を 3 回撮るのに 3 つ引っかかった。** 世界の種は時計（`--load` で固定）、生き物は止められない
  （セーブの `speed` 遺伝子を 0.02 にした）、時計を決めているのは `day_phase` ではなく `tick`。
* **撮り直した絵は 3 枚ではなく 5 枚。** `garden-guide.png` / `garden-guide-ja.png` の背景にも浮いたフィールドが写っていた。

## 一時停止と VM パネル（G9）

著者の指摘（2026-09-17）: 「VM パネルは一時停止しないと追えない」「`P` で世界は止まるのか？」→ 今の `P` はスクリプトの予算を 0 にするだけで、
Rust の規則（成長・移動・腹減り・昼夜・繁殖・餓死）は走り続ける。生き物は最後の `Velocity` で滑り、長く止めると餓死する。**間引きも 1 フレーム実行も要らない**（著者判断）。

1. **`P` は世界ごと止める**: 規則の chain・`day_night`・`hatch`・`court`・`startle` などを `paused` で止める（`is_still` と同じ形の run condition）。描画・カメラ・パネル・エディタ・セーブ/ロードは動く。
   `Velocity` の積分も止まる（滑らない）。再開時に `"night"`/`"day"` などを取りこぼさないこと（時計が止まるので自然に満たされるはず — 確認する）。Battle も同じ（弾・移動・タイマー）。
   selftest: 一時停止中 2 秒で位置・`Hunger`・`day` が動かない、再開後に動く。HUD の「paused (P)」の表示は既存のまま。
...
**実装で分かったこと**（`docs/worklog/2026-09-17-garden-G9.md`）:

* **止める道はもう在った。** 規則の chain には G3 から `run_if(is_still)`（セーブを読み戻す間は動かない）が
  付いていて、`is_still` に `Paused` を足すだけで `P` は世界を止めた。新しい仕組みは
  **時計を止める 1 行**（`hold_the_clock`: `sky.shift -= time.delta_secs()`）だけで、これは
  `restore_memory` が同じことを絶対値でやっているのを差分に書き直したもの。
* **「取りこぼさない」は時計が止まることの帰結だった**（計画書の予想通り）。`"night"`/`"day"` は
  `day_night` が位相をまたいだ瞬間にだけ publish され、動かない時計はまたげない。止める前に
  キューに入っていたものは、予算 0 で pop されないまま残る。
* **Battle で本当に止める必要があったのは時計。** `rule(after: 20, every: 2.0)` は
  `Rubevy.ask("clock")` と比べていて、これが `Time::elapsed_secs` のままだと再開の瞬間に
  取りこぼした回数ぶん `shrink` が連続する。`WorldClock` を足し、`status`/`radar` の時刻も同じにした。
* **待っている理由はフレームだけで当たる。** `Task::Queue#pop` は mrblib の Ruby なので、キューで待つ
  タスクには必ずフレームが在り、その 1 つ外のクラス名が種類を言う（`Rubevy::Subscription`＝イベント、
  `Rubevy::Entity`＝コンポーネント読み、`Rubevy::Proxy` かそれ以外＝ゲームへの質問）。
  `sleep` はネイティブでフレームを積まないので、`pop` が無ければ `sleep`。
  **当たらない場合が 1 つあり、隠していない**: タイムスライスを使い切って実行可能なだけのタスクは
  `sleep` と見分けが付かない（VM に読み取り専用の口が無い。`Task#status` は Ruby）。
* **イベント待ちはパネルの既定では出ない。** パネルが追うのは entity の `ScriptTask`＝行動アルゴリズムの
  タスクで、`on(:…)` はそれぞれ別タスク。どのイベントかも言えない（`event` はブロックのローカルで、
  そのタスクのスタックには無い）。
* **`P` は VM パネルを開かなくした**（計画書に明示が無い判断）。一時停止が世界のものになり、パネルが
  既定で閉じる以上、「止めると勝手にデバッガが出る」は残せない。
* **判定の 1 本を書き直した。** 再開側を「メーターの合計が減る」にしたら FAIL した — 草を食べている
  生き物は増えるし、子が生まれれば合計も増える。「同じ entity のメーターが変わった」に直した。
* **古い絵は 3 枚あった。** `garden.png` / `garden-night.png` / `garden-dusk.png` に**古い VM パネル**が
  写っていたので撮り直し（既定で閉じたので 2 パネルになる）。甲虫の 3 枚は
  リポジトリに無いモデルで撮られているので撮り直せない（著者判断待ちのまま）。
