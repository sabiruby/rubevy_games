...
| 09-22 F5 | **`[patch.crates-io]` を消した**（SabiRuby は crates.io の 0.6.1）。`pages.yml` の grep は rev → 版のタグ → 失敗、の 3 段に | 使われない patch は警告 3 行。空の ref は「既定ブランチ」なので**黙って VM の main から公開ページを建ててしまう** | `Cargo.toml` の `[workspace.dependencies]` と `.github/workflows/pages.yml` の `the SabiRuby Cargo.lock names` |
