...
| F3a | 未着手（2026-09-21 夜、著者「サイズは自由に変えられるようにしたい」— `map_tiles` の既定値をどうするかを聞いたことへの答え。**既定値を決める前に、自由に変えられる形が先**）。今は `factory.settings.txt` の `map_tiles`（正方形、起動時に 1 度、下限は黙って `.max` で切り上げ、ページには実質渡す口が無い）。**(1) 地図の大きさを `data.rb` の宣言に**（世界が何でできているかを言うファイル。F2 の「語を分ける」理由に従って新しい語、幅と高さを別々に — 正方形に限らない）。`ore_patch_radius` が設定に残っていた理由は「`map_tiles` の下限が VM の前に要る」だけだったので、地図が data stage の後に組まれるなら**畑の半径も一緒に `data.rb` へ**（F2a で `Ore::laid_out` は既に data stage の後）。`factory.settings.txt` の `map_tiles` は消す（同じことを言う場所を 2 つ残さない。古い設定ファイルにその鍵が残っていたら log で「`data.rb` へ移った」と言う）。**(2) 小さすぎる・大きすぎるは黙って直さず `data.rb:行` つきで断る**: 下限は今の導出（`Ore::smallest_map` を幅と高さそれぞれに）、**上限は「自由に」を縛る数を発明しない** — 本当に壊れる所（タイルの添字の型、`TilemapChunk` のタイル情報テクスチャの一辺 = WebGL2 の `max_texture_dimension_2d`、`Lanes` の確保）を調べ、壊れる所が言う数だけを上限にして出どころを書く。chunk を分ければ外れる上限なら、外すかどうかを測って決める（外さないなら「ここまで」と理由）。**(3) 大きい地図で遊べること**: 既定のカメラ（`camera_half_height`、ズームの範囲、パンの端）が地図の大きさから導かれること、`--stress` が自分で地図を決める道（計測の器）はそのまま、畑の置き方（今は 4 隅）が長方形と大きい地図でも意味を持つこと（大きい地図で畑が 4 つのままでよいかは決めて理由を。畑の数を言う口が要るなら `ore` の語に）。**(4) 既定値は 32×32 のまま動かさない**（著者の実機の数字が出るまで）。確認: 幅 ≠ 高さ、下限ちょうど、下限 −1（断られる）、大きい地図（上限の近く）でヘッドレス・窓・ブラウザが動き黒くならない、行の一覧は足した判定の分だけ増える、テスト。F5 で `data.rb` の再読み込みが入れば「遊びながら大きさを変える」= 書き換えて読み直す、になる形にしておく（表を作る関数は F2 から何度でも呼べる） |
| F4〜F6 | 未着手 |
