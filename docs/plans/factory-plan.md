...
| F7 | **済み**（2026-09-22、`f7` の 5 コミット: `c676e5c`（ガイドの小窓）・`0873a39`（カメラの `+`/`-`）・`04bf51c`（palette・ゴースト・次の 1 手）・`064b00b`（判定 9 行）・`a762809`（小窓を前面へ、判定のマウスを窓の割合に））。著者の 6 点に 1 対 1 で答えた（表は `docs/worklog/2026-09-22-factory-F7.md` の先頭）。**(1)(2) palette**（`factory/src/palette.rs`）: `what_the_digits_hold` の一覧を、**そのタイルシートのその絵**・名前・キーで並べる小窓。絵は**配列テクスチャを egui が描けない**ので、層 *n* の生バイトを `egui::ColorImage` にして 1 度だけ焼く（第 2 のシートを持たない）。`Hand` を書く口は 1 つ（`build::take_in_hand`。**message ではなく関数** — `Order` と違い「いつ意図されたか」を運ぶ必要が無く、message にするとゴーストが 1 フレーム遅れる）。**(3) ゴースト**（`draw::draw_ghost`）: **3 枚目の `TilemapChunk`**（いちばん大きい機械のぶんだけ。2048² の地図で 3 枚目の全面テクスチャを持たないため）を毎フレーム動かす。ベルトは**隣から曲がりを引いて**（`Flow::where_it_comes_in` を関数に切り出した）、腕は振る向き、機械は足跡ごと、半透明、**建てられない所は赤** — 赤の条件は `build::would_refuse` で、**クリックが従う規則そのもの**（`orders` も同じ関数を読むので「緑のゴーストのあとに拒否」が起きない）。ポインタが無い走行（コンテナ、まだ触っていないページ）は**見えている範囲の真ん中**に立つ。**(4) 次の 1 手**（`palette::NextStep`）: 格子から数え、`Grid::changes` が動いたときだけ。5 つ揃ったら control stage の `@saying`。**(5) キーの小窓**: 共有 crate（上の `shared-crate-plan.md` §6 の F7 の行）。**(6) ズーム**: `+`/`-`・palette の `[−] 3:1 [+]`・全体を見る・`Home`。**判定が見つけた穴**: 整数倍ズームの丸めが **1 ノッチを飲み込んでいた**（F1 から公開版まで `+`/`-` もホイール 1 ノッチも効いていなかった）。丸めが打ち消すときは**整数倍を 1 段**動かす形にした（`rung` / `half_height_of` / `step_the_zoom`、1:1 を真ん中に置いた梯子。新しい数は 0）。「全体を見る」は**地図が入る段**を採る（いちばん近い段だと 1 列はみ出す）。ガイドは「最初の線」の段落を palette・ゴースト・次の 1 手に合わせて書き直し、`+ -`・`Home`・`Build` の行を足した（16 → 18 行）。**フォントを切り直した**（81,480 → 86,592 B、417 グリフ。Factory の日本語は guide_text.rs に置く — `tools/subset-font.sh` が読む 4 ファイルの 1 つだから）。確認: 警告 0、テスト **121 → 127**、`unsafe` 0、wasm 3 本、**9 通りのうち 8 通りが一覧と diff 空**（factory の窓 39 → **48**・ブラウザ 40 → **49** は足した 9 行ちょうど、箱庭 13/47/48 と Battle 4/32/33 は**動かず**）、`web/build.sh all` + Playwright で 3 ページ pageerror 0・requestfailed 0・黒 0、`--shot` は `docs/factory-palette.png`。記録は `docs/worklog/2026-09-22-factory-F7.md` |
