...
| F2a | 未着手（2026-09-21 著者「F3 の前に変える」）。**ベルト上の位置を f32 から整数の刻みへ。** 1 タイル = `TILE_PX` = 16 刻み（= 素材 1 画素。`TILE_PX` は素材の寸法なので不変量）。位置は刻みの整数、隙間は `16 ÷ items_per_tile` 刻みで、**`items_per_tile` は 16 の約数（1・2・4・8・16）だけ** — それ以外は data stage が `data.rb:行` つきで断る（F2 の `deserialize_with` の作法）。速さは `tiles_per_second × 16` 刻み毎秒で、フレームごとの端数は**失わずに持ち越す**（どこに持つか — ベルトごとか工場に 1 つか — は「同じ速さのベルトが同じフレームに同じだけ進む」を満たす方を選んで理由を残す）。確認: 「隙間 N 個の列に N+1 個」が**通る値の全部で厳密**（F1 の 7 値のテストを 16 の約数に取り直し、3 と 5 と 2.5 が行つきで断られるテスト）、運ぶ・詰まる・合流・採掘機・機械の既存のテストが全部通る、長時間の走行で「掘った数 = 箱 + ベルト + 機械の中」が 1 個も狂わない、描画（`item_at`）が刻みから画素へ丸めなしで行く、行の一覧は diff 空（判定の文面は変えない）。`--stress` で step の µs を前後交互に取り、遅くなっていないこと（静かな機械で）。**既定値は動かない**（`items_per_tile: 2.0` は 8 刻み。`data.rb` の綴りを整数 `2` にするかは決めて理由を残す）。あわせて計画 §7 の「F3 の最初に」の 3 件のうち **`ore_per_tile` を `data.rb` へ**をここでやる（建てる依頼のメッセージと `checks_end_the_run()` への乗り換えは F3 の最初のまま） |
| F3〜F6 | 未着手 |
...
| 09-21 F2 | **`Declarations::take` の表に行番号が無い**: 宣言をまたぐ検査（無いアイテム、無い `made_in`、`size` と `sprite` の枚数）に言える行が無く、games がソースを引いて探す 10 行の迂回（`data::line_of`）を入れた。「始まる行」を返すので serde の「終わる行」と食い違いうる | sabiruby `serde/src/declare.rs` | VM（sabiruby-serde） | **著者判断待ち**（sabiruby の公開 API に足す話: `take` が行を返すか `line_of(&name)`）。迂回は害が無いので急がない |
| 09-21 F2 | **`Options::symbol_keys` が Hash のキーに効かない**（struct のフィールドにしか通っていない）。Symbol で書いた名前が String で返る: `recipe_of(:x)[:in]["iron_ore"]` | sabiruby-serde `ser.rs` の `Serializer::key` | VM（sabiruby-serde） | **F3 の前に直したい** — インサータの Ruby が `recipe_of` を読む（利用者に見える綴り）。sabiruby の計画に足す。著者に報告 |
...
| 09-21 F2 | **クリックに「何を持っていたか」が載っていない** — 判定とクリックの順序のちらつき（窓でだけ FAIL 2、F1 からの穴）の根。F2 は順序 1 行で直した | `factory/src/build.rs`、`WorldClick` | ゲーム固有 | **本体が原則から決めた: F5 の前に、建てる依頼を「場所 + 何を + 向き」の自前のメッセージにする**（`WorldClick` は共有 crate のものなので載せない。エディタとブループリントが同じ口を使う）。F3 の最初に |
...
| 09-21 F2 | `ore_per_tile` = 60 の出どころは「箱 1 杯ぶん」で、その箱が `data.rb` へ行ったのでファイルをまたぐ導出になった（追従しない）。(ii) `ore_per_tile` も `data.rb` へ移すのが原則に合う | `factory.settings.txt`、`ruby/data.rb`、`docs/numbers.md` §9.2 | ゲーム固有 | **本体が原則から決めた: (ii)**（遊びの数は Ruby。既定値 60 は動かない）。F3 の最初に |
| 09-21 F2 | 位置を整数の刻みで持つ模型: 1 タイル = 16 刻み（= 素材 1 画素）、`items_per_tile` は 16 の約数に限られ data stage が行つきで断れる。N+1 が厳密、描画の丸めも消える。失うのは 2.5 のような値と `belts.rs` の書き換え。セーブ（F5）の前なら安く、F3 の後はインサータのぶん高い | worklog F2 の報告 §9(a) | ゲーム固有 | **著者判断待ち**（模型の変更と、利用者が書ける値の範囲が変わる） |
