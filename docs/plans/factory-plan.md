...
| F4 | **済み**（2026-09-22、`factory` の `95c162f`・`a5fdc1a` と docs のコミット）。**3 本目の Ruby が入った段階**（data stage・インサータに続く control stage）。(a) **先に F3・F3a の気づきを 3 件**: 毎フレーム地図全体を舐める 2 か所を消し（尻尾は建物のあるタイルだけ、個数は `Lanes` が持つ。そのために `Lanes::of` を非公開にし出入口を 4 つに絞った）、**空の地図の step は 32² / 512² / 2048² のどれも 1 µs**（F3a は 4 µs / 0.6 ms / **5.9 ms**）になった；機械を持っているとき `R` は何もしない（理由を log が 1 行言う）；判定が「地図の真ん中は空いている」に寄りかかっていたのを **`somewhere_clear`（鉱石が無く何も建っていない長方形を真ん中から外へ探す）**に置き換えた（採掘機の判定のタイル 1 枚も同じ穴だった。既定の地図では今までと同じ場所に建つので `--shot` の絵も変わらない）。(b) **イベントは 5 つ、粒度は機械**（`built`/`removed`/`crafted`/`delivered`/`jammed`）で、**1 フレームに種類ごと 1 件**（何が・幾つ・どこで の平たい 4 つ）。これは購読 1 本 **64 件/フレーム**からの逆算で、**種類の数は `data.rb` が決める**（item 3 + 建物 6）ので**データファイルに書ける値では天井に届かない**。1 個ずつ publish すると納品だけで腕 60 台で当たる。**搬送物 1 個ごとのイベントは無い。** (c) **詰まりの定義**は「作った物を持っていて、かつ次の材料も持っている機械」— ベルトの詰まりは動いている工場の正常な姿なので publish しない。状態ではなく**縁**（そうなったフレーム）を 1 件にする。`machines::craft` の既存の分岐で判定するので追加の走査は 0。(d) **`goal deliver: { gear: 60 }`** = 鎖 1 本（毎秒 1 個）を 1 分 = 箱 1 杯。**目標は `control.rb`**（遊び方であって世界の材料ではない）。(e) **スクリプトは ivar を読まれることで答える**（`@won`・画面の 1 行・イベントごとの `@seen_*`・`@dropped`）: 箱庭が `@asleep` を読むのと同じ道で、**request を 1 つも増やさない**（納品ごとに `ask` すると 1 件 1 フレーム）。**control がゲームに働きかける口は `win!` だけ**（建てる口は作らない — 目標は言うこと、建てるのはプレイヤーの手）。(f) **top level に書く DSL は例外の行が取れない**ことが分かり、**ゲームがファイルを 1 行のメソッドで包む**ことで解いた（`in_a_method`。実測: top level の `nothing.at.all` が `control.rb:3`。包む前は `control.rb:?`）。代償は閉じ忘れの構文エラーの行がファイル末尾より少し先になること。(g) **計測**（release、`--arms N`、control 有り/無しを交互 2 巡、別の担当が同時に走っていたので µs は参考）: 340 台で命令 p50 1,399 → 1,579、3,400 台で 11,983 → 11,947 — **control は毎フレーム約 200 命令で、工場が 10 倍になっても増えない**（1 フレームに種類ごと 1 件だから）。**`dropped` は全部 0**（VM 側もスクリプト側も）。予算 39,000 に対し 340 台で p95 2,443 = 6%。(h) `headless_seconds` 30 → **66**（判定の上限の和が 20 → 44 秒）。判定は**別システム**（`control_checks`。system の引数 16 個の壁）。テスト 110 → **114**、判定 19 → **24**（窓 27・ブラウザ 28）。確認: 警告 0、`unsafe` 0、ヘッドレス 2 回が互いに diff 空、窓 27 行 FAIL 0、ブラウザ 28 行 pageerror 0・requestfailed 0・絵に黒 0、`--shot` の絵も黒 0、箱庭のヘッドレスは一覧と diff 空。記録は `docs/worklog/2026-09-22-factory-F4.md` |
| F5 | **済み**（2026-09-22、`factory` の `fb7d8b1`・`de0eae9`・`2bd0c0f`・`cca13f1`・`6a8abcd` と docs のコミット）。**窓が一式そろった段階。** (a) **土台を今日のものに**: workspace の SabiRuby を版で（`0.6` / `0.3` / `0.2`）、rubevy を `c9b52a3` へ。`cargo update` の警告が「patch は使われていない」と言ったので **`[patch.crates-io]` を消した** — crates.io の 0.6.1 にこの workspace が呼ぶものは全部ある。`pages.yml` の grep（lock から VM の rev を探して Playground のコンパイラを建てる）は**当たらなくなり、空の ref は「既定ブランチ」なので黙って VM の main から建ててしまう**ので、rev → 版のタグ（`v0.6.1`）→ 失敗、の 3 段にして使った ref を log に出すようにした。(b) **`Held` に乗り換え**: `Arms::waiting` の索引と後始末、prelude 2 本の `said_at` と `@broke_at`（43 行）、`ivar_get` 2 か所が消え、**両方の `compile` の 2 パス目も消えた**（`Script::with_prelude_lines` が「著者の行は何行目から」を Ruby の外に置いたので、生成ブロックが自分の長さを持つ循環ごと消えた）。**再 raise が例外を `RuntimeError` にしていた**ことが分かり、判定の文面が本物の `NoMethodError` に変わった（`Task::Overrun` の穴も塞がった）。(c) **エディタは 3 ファイル**。`data.rb` の Apply は「読み直す」で、**世界が今の宣言に合うなら組み直さない**（ベルトは運び続ける）、**合わないなら 1 クリック確かめてから組み直す**（地図の大きさ・鉱石・アイテム/機械/レシピの並びが「世界が索引にしているもの」）。読めない `data.rb` は `data.rb:行` を出して**世界に触らない**。**これで地図の大きさがブラウザでも遊びながら変えられる**（F3a が宣言にした数を、F5 が回せるつまみにした）。(d) **セーブ**: 格子とレーン（F3 が腕の状態を格子に置いたので難しい半分が最初から無い）、鉱石、端数、メモリ内のスクリプト、**スクリプトが覚えているもの**（腕の `@memory`、control の `@got`/`@won` — 勝ち 5 個手前で保存した工場は勝ち 5 個手前で戻る）。`data.rb` の印を持ち、別の `data.rb` のセーブは**読まずに断る**。**save → load → save がテキスト一致**（判定が世界を止めてやる — ベルトは 1 フレームに半刻み進む）。(e) **egui の HUD**（F4 の `bevy_ui` の 1 行を置き換え）、VM パネル（`F2`）、ガイド（`H`、英日。フォントを切り直して 81,480 B / 396 グリフ、豆腐 0）、`PanelSettingsPlugin`。(f) **`P` は世界ごと止まる** — 箱庭の G4 の間違い（スクリプトだけ止めると世界が動き続ける）を写さない。`Paused` はどの走行にもあり、工場の step の run condition。**建てるのとエディタは動く**（詰まった線を見て、要ると分かったベルトを置けないポーズは誰も使わない）。(g) **Ruby からカメラ**: `look_at(column, row)` が rubevy の任意の層 `Rubevy::Camera`、勝つと納品した箱へ寄る。`CameraView` との繋ぎは**発散せず収束する**形（`focus = t - shift`）。**`Transform` と `Camera2d` を `register_type` していなかった**ので最初はカメラが見つからなかった。確認: 警告 0、`unsafe` 0、テスト 114 → **121**、判定 24 → **31（窓 39・ブラウザ 40）**、3 本 9 通りが一覧と diff 空。記録は `docs/worklog/2026-09-22-factory-F5.md` |
| F6 | **済み**（2026-09-22、`factory` の `de6de52`・`94575c5`・`58d3b1f`(merge) と docs のコミット）。**3 本目が公開された段階。** (a) **公開の配線は 2 行**: `web/games.sh` の `GAMES_ALL` に `factory` の 1 語と、`web/index.html` の `<li>` のコメント解除。F0 から**わざと対で**止めてあったもので、片方だけでは「リンクの無いページ」か「行き先の無いリンク」になる。**`pages.yml` は 6 段階を通じて 1 度も変えていない** — ゲームの名前を 1 つも書いていないから。ただし F0 に書いた**ブロックのうち 2 つが古くなっていた**: 鍵の一覧に `F9` が無く（F5 がセーブを入れた。`F5`/`F9` の意味は箱庭と同じ）、2 つの注記が「まだコンパイラを呼んでいない」「パネルはまだ無い」と言っていた。どれも**ビルドでは捕まらない**（`view-source` を開いた人が読む散文）。(b) **予算の実測（§9.8a）**: `--arms 340` / `3000` を交互 2 巡 + 向きを入れ替えて 2 巡（計 8 走行）。**命令数は決定的**で、340 台 p95 **2,227**（予算の 5.7%）、3,000 台 p95 **14,971**（44%）、**持ち越しも `dropped` も 24 報告すべて 0**。rubevy の 3 歩を今日の数でやり直すと毎 ms 9,019 命令 × 4 ms = **36,000** で、39,000 との差 8% は **tick µs 自身のばらつき 25% より小さい**。**自分の測定のぶれの内側なので既定値は動かさない。** (c) **ブラウザの起動の値段**: 3 つの同期コンパイルがそれぞれ ms を言うようにし（`data.rb` は F2 から）、3 回で合計 **32.4 / 30.6 / 13.3 ms**、`page.goto` から最後のコンパイルまで **0.80〜1.07 秒**。ばらつきはファイルではなく**コンパイラのモジュールが温まる**ぶん（いちばん長い `inserter.rb` が 3 番目なので 0.7 ms）。(d) **著者が実機で測る道**: `stress:` の行に**地図の大きさと GPU のアダプタ名**を足し、手順を `docs/verification/factory-on-a-real-gpu.md` に 5 行。**URL から地図を渡す口は作らなかった** — 地図は `data.rb` の宣言で、綴りを 2 か所にしない。`?arms=1` が「`data.rb` の地図のまま測る」唯一の形（`?stress` も `?arms=N>1` も地図を下から押し上げる）。実測: SwiftShader で 48×48・アイテム 4,000 個が **step 100 µs / フレーム 360 ms**、`device_type: Cpu`。**2 桁の差が全部描画**で、F1 の「ブラウザでは測れない」は今も正しい。(e) **CI の 3 つの固定**（lock の SabiRuby・playground の `SABIRUBY_REF`・games の `PLAYGROUND_REF`）が揃っていることを、CI の「the Ruby compiler module」の段を**スクラッチパッドで再現して**確かめた（`5337b2f` + `v0.6.1` で通る、2,448,533 B）。`docs/web.md` に「Three pins, and they are one pin」として、1 つ置き去りにすると何が起きるか（09-22 の赤 2 回・無言のまま公開ページは前のまま）と、動かす順番を書いた。(f) docs: `web.md` の表を**3 本とも 4 列取り直し**（factory は 35,668,110 B / 10,942,946 gz、素材 11,326 B。Battle の +1.1%、箱庭の −1.0%）、`factory.md` を今の実物に、`docs/README.md` の目次、`CREDITS.md` と `.gitignore` は**変更不要だった**。確認: 警告 0、テスト **121**、`unsafe` 0、9 通りの走行が一覧と diff 空。記録は `docs/worklog/2026-09-22-factory-F6.md` |
| F7 | 未着手（2026-09-22、著者が公開版を実際に触っての指摘 5 点: 「今何を設置可能なのかわからない」「ショートカット以外でも設置物を選べるサブメニューが欲しい」「今どの向きで機材を設置しようとしてるのかわからない」「どういう順番で機材を設置するのかわかりにくい」「ヘルプのショートカットは移動可能なミニ Window で常時表示も選べるようにしたい」）。まとめると **(a) 手の状態が見えない**（何を持っているか・どちらを向いているか・何が持てるか）と **(b) 案内が「読む壁」しか無い**（開くと画面の真ん中を塞ぐ 1 枚）。**(1) 建てるものの palette**（egui、常時表示の小さい窓）: `data.rb` から `what_the_digits_hold` が出す一覧を、絵（タイルシートのそのアイコン）と名前とキーで並べ、クリックで手に持つ。持っているものはハイライト。手ぶら（撤去）も 1 枠。`R` の向きを矢印で。**(2) 格子の上のゴースト**: マウスのタイルに、持っているものを半透明で、その向きで描く（ベルトは `belt_picture` の絵、インサータは腕の向き、2×2 の機械は足跡ごと）。建てられない所（鉱石の無い所に採掘機、地図の外）は赤く。**(3) 順番の案内**: 「最初の線」（採掘機 → ベルト → 箱、次にかまど + 腕）を palette の下の 1 行の**進みぐあい**で（今の工場に採掘機が無ければ「2: まず鉱石の上に採掘機」、ベルトが無ければ「1: そこから伸びるベルト」…と、次の 1 手だけを言う。control stage の `built` と同じ数から出す。全部揃ったら消える）。**(4) キーの一覧を小さい窓に**: `Guide` を「本文（H で開閉）」と「キーの表（移動できる小窓、常時表示のオンオフを `factory.settings.txt` と `remembered` で覚える）」に分ける。共有 crate の `Guide` の話なので `games_shell` に入れ、箱庭と Battle も同じものを得る（`GuideStyle` に鍵、既定は今と同じ = 表は本文の中）。**数**: 窓の大きさ・位置・透明度は `GuideStyle` / 新しい `PaletteStyle` の設定に、既定の出どころを 1 行で。**確認**: 判定を足す（palette のクリックで手が変わる、ゴーストが向きどおりの絵、進みぐあいの文が工場の状態に追随、キーの小窓の表示を覚える）。3 本の 9 通りが一覧と diff 空（(4) で箱庭と Battle の行が変わるならその分だけ）、`--shot` に palette とゴーストと小窓が写る 1 枚、ブラウザで pageerror 0。**著者の決定（2026-09-22）: キーの小窓の既定は 3 本ともオン。ゴーストの色は本体に任せる（建てられる所は半透明の元の絵、建てられない所は赤みがかった半透明）。追加: 「マップの拡大縮小もしたい」** → **(5) ズームを見える・届く形に**: 今はホイールだけ（`camera::zoom_by`、ブラウザやトラックパッドでは気づけない）。`+` / `-`（と `=`）のキー、palette の中の **[−] 1:2 [+]** のような表示つきのボタン（今の倍率を「画面 1 px が絵の何 px か」で。`snap_zoom` の整数倍と同じ物差し）、**全体を見る**ボタン（地図全体が入る倍率へ、F3a の「全体まで引ける」の仕上げ）、`Home` で既定の見え方に戻る（共有 crate に既にある）。ボタンもキーも共有 crate の `zoom_by` の 1 つの口を通し、限界も同じ。ズームの中心はマウスのタイル（ホイール）／画面の中心（キーとボタン）。共有 crate の `CameraControls` に足すので箱庭と Battle にも届く（箱庭の 3D は「全体」の意味が違うので `showing` の既定に戻る、で可）。判定: キーとボタンで倍率が変わり、限界で止まり、`Home` で戻る。**著者の指摘に 1 対 1 で答える表**を worklog に |
...
| 09-21 F1 | **「正でない設定を読み込み時に断る」は 3 本とも要る形**: F1 は `positive` / `counted` を factory の中に書いて `.max(0.001)` を 3 つ消した。箱庭と Battle にも同じ形の数が無いか見てから `games_shell::Settings` へ | `factory/src/main.rs`、`games_shell::Settings` | 共有 crate | **S11 (2) で済んだ**（2026-09-22）: `games_shell::Settings::positive` / `counted(key, least, default)`。factory の 12 行は消え、箱庭・Battle・共有 crate の同じ形 20 か所も同じ口へ。断りは `SettingsRefusalsPlugin` が `Startup` で言う（`main()` の `warn!` は出ない） |
...
| 09-21 F1 | `--shot` と判定の両立を F1 は factory の中だけで直した。S9 が共有 crate に `checks_end_the_run()` を入れるので、main に入ったら factory はそれに乗り換える | `factory/src/main.rs` の selftest、`games_shell::checks` | 共有 crate | **S11 (1) で済んだ**（2026-09-22）: factory は `games_shell::checks::Errands` に乗り換えた。F4 が `CHECKS_EXIT_WHEN_DONE` を直に見ることはもう無い |
...
| 09-21 F1 | `wasm-opt` が既定の PATH に無く（`~/.local/binaryen-version_132/bin`）、`web/build.sh` が黙って「縮めなかった」で進む。`web/build.sh all` の後に 1 本だけ建てると feature の解決が変わって wasm の依存を全部建て直す（約 20 分） | `web/build.sh` | 道具 | **S11 (3) で済んだ**（2026-09-22）: `~/.local/binaryen-*/bin` を探し、無ければ最後にもう一度はっきり言う（止めない理由は worklog §3）。CI の道は不変 |
...
| 09-21 F2 | `--shot` と判定の終わり方は 2 方向ある（F1「判定が済んでも絵を待つ」、F2「絵を撮っても判定を待つ」）。S9 の `checks_end_the_run()` は片方だけ | `games_shell::checks`、3 本の `take_shot` | 共有 crate | **S11 (1) で済んだ**（2026-09-22）: 両方向が `Errands` に。`take_shot` は絵を撮るだけになり、終わりを決めるのは `end_the_run` の 1 か所 |
...
| 09-21 F3 | **判定が「1 フレーム後に見る」と書いてあると、窓で通ってブラウザで落ちる。** F3 で 2 回踏んだ（パネルが開くのを待つ所と、VM がプログラムを持つのを待つ所）。S7 の「秒ではなく条件で待つ」は**フレームでも同じ**で、`docs/verification` に 1 行書く価値がある | `factory/src/main.rs` の `PANEL_WAIT_FRAMES` | 確認の作法 | **S11 (4) で済んだ**（2026-09-22）: `selftest-lines.md` の作法の節の 1 つ目 |
...
| 09-21 F3 | 判定が「1 フレーム後に見る」と書いてあると、窓で通ってブラウザで落ちる（F3 で 2 回）。S7 の「秒ではなく条件で待つ」はフレームでも同じ | `docs/verification`、`implementer.md` | 確認の作法 | **S11 (4) で済んだ**（2026-09-22）: `selftest-lines.md` の「What a check has to get right」。`implementer.md` への追記案は S11 の報告に文面で |
...
| 09-21 F3a | **`main()` で `warn!` を書いても何も出ない** — `LogPlugin` は `DefaultPlugins`/`MinimalPlugins` の中なので、`App` を建てる前のログは subscriber が無く捨てられる。F3a は「移った設定の鍵」の警告をそこに書いて、窓の走行で何も出ずに気づいた（`Startup` の system へ移した） | 3 本の `main()`、`docs/verification` | 確認の作法 | **S11 で済んだ**（2026-09-22）: 作法の節の 4 つ目に書き、(2) の断りはこの理由で `Startup` から言う形にした |
...
| 09-21 F3a | **`--shot` と判定を同時に使うと `done` の行が 1 行増える**（`CHECKS_EXIT_WHEN_DONE && shot.is_none()`）。`selftest-lines.md` の「窓 22 行」は `--shot` 無しの数である | `factory/src/main.rs` の selftest の末尾、`docs/verification/selftest-lines.md` | 確認の作法 | **S11 (1) で済んだ**（2026-09-22）: `--shot` つきの走行が増やす行は `selftest-lines.md` に書いた。**判定より先に絵が済んだ走行は `done` を言わなくなった**（factory の既定 8 秒がその形）|
| 09-22 F4 | **top level に書く DSL は例外の行が取れない。** インサータ（`inserter "X" do … end`）は中身がブロックなので prelude の `rescue` の中だが、`control.rb` は top level なので `run_control` に入る前（プログラムの読み込み中）に走る。ゲームがファイルを 1 行のメソッドで包んで解いた（`in_a_method`）が、**包み方は `Program::new`（prelude + body + tail）の外側の話**で、`Program` に「body をメソッドで包む」選択肢があれば次のゲームが同じ 10 行を書かずに済む | rubevy の `Program`、`factory/src/control.rs` | rubevy（汎用の口）／本の素材 | **報告のみ**（本体が決める） |
| 09-22 F4 | **包むと構文エラーの行がファイル末尾より先になる**（3 行のファイルの閉じ忘れが `control.rb:7`）。F2 の「半端な行はパーサが諦めた所で報告される」の延長で、包んだぶん `end` と `run_control` が後ろに増えた。例外の行は厳密なので、判定はそちらを見ている | `factory/src/control.rs` | ゲーム固有／本の素材 | **報告のみ** |
| 09-22 F4 | **`ScriptWorld::dropped` は VM 全体の数**で、どの購読が落としたかを言わない。今は control が 1 本なので困らないが、購読を持つスクリプトが 2 種類になると「誰の穴か」は `Subscription#dropped` を全部集めるしかない（prelude がやっている）。rubevy が名前ごとの内訳を持てば HUD が 1 行で書ける | rubevy の `ScriptWorld::dropped` | rubevy | **報告のみ** |
| 09-22 F4 | **16 引数の壁に 2 度目に当たった**（F3 は `Editor` を `Crew` に入れて避け、F4 は判定のシステムを分けた）。3 度目は「判定を小さく分ける」ほうが素直で、F5 は窓の判定が増える | `factory/src/main.rs` の `selftest` / `control_checks` | ゲーム固有／F5 | **報告のみ** |
| 09-22 F4 | **`docs/factory.md` の「What one step of the factory is」が F3 の時点で古かった**（「4 パス」と書いて 6 つ並べ、F3 が消した `deliver` がまだ入っていた）。F4 が直した | `docs/factory.md` | 文書 | **済み**（F4） |
| 09-22 F5 | **`ScriptEnded::at` のファイル名は VM の言う名前**で、ページでは `playground.rb` になる（コンパイラのモジュールがどのプログラムもそう名づける。F2 の気づきと同じ根）。F3 の prelude は名前を Ruby の中で組み立てていたので当たらなかった。**乗り換えは PC で通りブラウザで文面が変わる**という形の落ち方をする | rubevy の `ScriptEnded::at`、sabiruby-playground の `sabi_compile` | playground と共有 crate | **factory の中で直した**（呼ぶ側の名前を使う）。口としては F2 の気づき「ブラウザのコンパイラにファイル名を渡す口が無い」と同じもので、そちらが直れば消える |
| 09-22 F5 | **偽のキーは押したフレームだけのもの**（`ButtonInput::clear` が次のフレームの頭で `just_pressed` を消す）。キーを読むシステムより前に判定を並べないと「押していない」のと同じで、**並べる口が無い**（`guide_keys` が private だった）。`selftest-lines.md` の「秒ではなく条件で待つ」に並ぶ作法 | `crates/games-shell/src/guide.rs`、3 本の判定 | 確認の作法／共有 crate | **`pub` にした**（1 語）。作法は `selftest-lines.md` の Factory の節に書いた |
| 09-22 F5 | **`Paused` が「世界」と「スクリプト」の 2 つに割れた。** 箱庭と Battle の `P` は VM の予算だけで、箱庭は G4 でそれが足りないと分かって世界の側に run condition を足している。3 本目も同じ形になったので、**「止まる」を共有 crate の語にできる**（`games_shell::Paused` と run condition 1 つ） | `factory/src/window.rs`、`garden/src/window.rs`、`sabibots/src/main.rs` | 共有 crate | **報告のみ**（本体が決める。利用者が 3 本になった） |
| 09-22 F5 | **セーブの形が 2 本目になった。** 箱庭の `GardenSave` と factory の `FactorySave` は、先頭の `version`、版だけ先に読む `read_save`、`platform::read`/`write` 越しの PC とブラウザ、`--save` / `--load`、HUD のボタン 2 つ、note の 1 行、までが同じ。**違うのは中身だけ**（どちらも「この repo の形式」であって共有 crate の型ではない） | `garden/src/main.rs`、`factory/src/save.rs` | 共有 crate の候補／本の素材 | **報告のみ** — 共有できるのは `version` の読み方と `SaveFile`/`Asked`/`SaveNote` の 3 つの Resource で、`Save<T>` のような型ではない |
| 09-22 F5 | **factory は `register_type` を 1 行も書いていなかった。** 箱庭と Battle は書いている。rubevy が component を型レジストリ越しに触るので、Ruby に見せたいものは名指ししなければならず、**見せ忘れは「nil が返る」という静かな形で出る**（カメラの層がカメラを見つけられなかった） | `factory/src/main.rs`、rubevy `docs/host-api.md` | 文書／本の素材 | **factory の中で直した**（2 行）。rubevy の文書が「`MinimalPlugins` は `Transform` を登録しない」と言っている所の隣に「bevy の型でも、その plugin を足していなければ登録されていない」があると次のゲームが踏まない |
| 09-22 F5 | **`--shot` が窓の中身を 1 枚に収められない。** ガイドは `--guide`、VM パネルは `--vm`、エディタは「判定が腕を開いたころ」でしか出ないので、HUD・エディタ・ガイドが 1 枚に揃った絵は無く、3 枚に分かれる | `factory/src/main.rs` の `take_shot`、3 本の `--shot` | そのゲーム固有／道具 | **報告のみ**（F6 の公開用の絵をどう撮るかで要る） |
| 09-22 F5 | **`docs/web.md` に Factory の行が無い**（表もサイズも）。F6 の仕事だが、今の数はある: `factory/pkg/game_bg.wasm` 35,665,656 B、`factory/assets` 11,326 B / 4 ファイル、ガイドのフォント 81,480 B（3 本とも増えた） | `docs/web.md` | 文書 | **済み（F6）**: 表を 3 本とも 4 列取り直した（factory 35,668,110 / 10,942,946 gz）。Factory の節、3 つの固定の節、鍵の表の 3 本目も足した |
| 09-22 F6 | **`web/games.sh` の散文は、ゲームが無いうちに書くと 6 段階ぶん古くなる。** F0 が書いた Factory のブロックは、鍵の一覧に `F9` が無く（F5 がセーブを入れた）、2 つの注記が「まだコンパイラを呼んでいない」「パネルはまだ無い」と言ったまま公開されかけた。**ビルドでは捕まらない** — `{{…}}` が埋まっているかしか見ていないし、鍵の一覧は「多すぎても少なすぎても動く」。公開前に 1 度読み合わせる以外に道が無い | `web/games.sh`、`web/build.sh` の `write_page` | 道具／文書 | **F6 が直した**。仕組みとしては、鍵の一覧をゲームの `KeyCode` から生成する道はある（3 本とも `just_pressed` を数か所で読むだけなので静的には拾えない）。**報告のみ** |
| 09-22 F6 | **`?stress` の走行はページで終われない**（`watch_the_frames` の `exit.write` は `Errands::left()` を見るので、ページでは永久に `Some("a page has nothing to exit to")`）。結果として**報告が 1 秒に 1 行、いつまでも出る** — 著者が測るには好都合なので直さなかったが、「走行が頼まれること」は S11 の数え方では 2 つ（判定と絵）で、**計測は 3 つ目なのに `Errands` に入っていない**。PC では `stress.said >= STRESS_REPORTS && errands.left().is_none()` が正しく働くので実害は無い | `factory/src/main.rs` の `watch_the_frames`、`games_shell::checks::Errands` | 共有 crate | **報告のみ**（本体が決める。3 つ目の errand を足すなら 3 本に効く変更） |
| 09-22 F6 | **`--shot` の絵は公開の顔には使えない**（F5 の気づきのとおり 3 枚に割れる）ので、F6 は**絵を 1 枚も足さなかった** — README にも入口のページにも**絵を置く流儀がそもそも無い**（画像があるのは `docs/*.md` だけ）。ブラウザの判定の走行が撮るスクリーンショットは、判定の最後が世界を組み直すので**空の工場**が写る | `README.md`、`web/index.html`、`factory/src/main.rs` の `take_shot` | 著者判断 | **本体が原則から決めた: 足さない**（無い流儀を 1 本目のゲームで作らない）。要るなら 3 本ぶん一度に決める話 |
