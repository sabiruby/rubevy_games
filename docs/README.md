...
| [garden.md](garden.md) | Garden: the second sample — a 3D world whose creatures read and write their own ECS components from Ruby by name. What exists after G0, G0a, G1 and G2: the component table, the rules, the CC0 models and their animations, the DSL the two creatures are written in, the wheel a reflex takes, the four questions, the `Genome` a Rust struct and a Ruby class both are (with the line count of the same class written by hand), breeding, and the numbers beside Battle's |
