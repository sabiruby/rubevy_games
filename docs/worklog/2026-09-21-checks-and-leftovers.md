# 判定の側 (d) と、S5b-1〜4・S6〜S8 が送った宿題（S5b-5）

計画書 `docs/plans/shared-crate-plan.md` の段階 **S5b-5**——S5b の最後。
先に読んだ記録は `2026-09-20-window-check-flakes.md`（S6）、`2026-09-20-window-check-fixes.md`（S7）、
`2026-09-20-writes-landing-in-a-pause.md`（S8）、`2026-09-21-numbers-garden-settings.md`（S5b-3）、
`2026-09-21-numbers-garden-play.md`（S5b-4）、`docs/verification/selftest-lines.md`、
`tools/fixedlines.sh`、`docs/numbers.md`。
着手時は main と同じ `02d3635`、作業は worktree `rubevy_games-wt-shared` の `shared-crate`。

**著者が決めていたこと**（2026-09-21）: 生き物の VM の予算 `script_budget` の既定を
**案 A — `1.74 × 23,686` ≈ 41,000** に。それ以外の既定値は動かさない。

---

## 0. 着手前に取った基準

機械は空いていた（`vmstat 1 3` の idle **99〜100%**）。隣の worktree に前の担当の待ちのシェルが
2 つ残っていたが、cargo も docker も動いていない。

`cargo build --workspace --all-targets` 警告 0、`cargo test --workspace` **58 通過**
（21 + 10 + 18 + 6 + 3）。

前の版は `git worktree add --detach`（`git stash` は使わない規則）で `c17abe1` を
`rubevy_games-wt-s5b5-before` に出し、**別の `CARGO_TARGET_DIR`**（その worktree の `target/`）で
建てた。`md5sum` は前 `e2bb9ff1…`／後 `87a19c9a…` で別のバイナリである。docker 側も
**worktree ごとの volume**（下の §1）なので、`/target/release/garden` は前 `6c8a675d…`／
後 `8e019b96…` と別物であることを volume の中で確かめた。

---

## 1. 道具を先に直した — docker の target volume と環境変数（C-2）

**順番を変えた理由**: この段階の確認は窓の走行（docker）に寄りかかっている。ところが
`docker/build.sh` と `docker/run.sh` はどの worktree も `/app` に mount し、`/target` は
**全 worktree で 1 つの volume** だった。つまり

* cargo からはどの worktree も同じ絶対パスに見え、別の worktree の build が「何も変わっていない」
  と読まれる（印は異様に速い `Finished`。S7 の記録にある）。
* そして `/target/release/garden` は**最後に建てた worktree のもの**である。窓を開けても、
  画面には別のブランチのゲームが出ているかもしれず、それを言うものが何も無い。

2 つ目の方が悪い。**自分の前後の比較を、その罠の上で取るわけにいかない**ので、A より先に直した。

`docker/volumes.sh` を新しく置き、`build.sh` と `run.sh` の両方が source する
（同じ 1 行を 2 か所に書かないため）。volume 名は checkout のディレクトリ名から決める。
古い `rubevy-games-target` は**消さずに置いた**（`docker volume rm` は使う人が決める）。
cargo の registry は共有のまま——crate の原本はどの worktree でも同じものである。

値段は「worktree ごとに 1 回の全 build」で、この機械では **351 crate を 3 分 41 秒**だった
（24 コア）。`before` の worktree も自分の volume で 3 分 03 秒。別のブランチのゲームを
知らずに走らせる値段より安い。

**環境変数**: `docker/run.sh` は `<GAME>_SELFTEST` **だけ**を渡していた。判定は自分のつまみも
環境から読む（`GARDEN_RELOAD_AT`）ので、渡されないつまみは「効かなかったつまみ」と見分けが
つかない——S7 の担当はそれで 4 走行を捨てている。`<GAME>_` で始まる名前を**探して**全部渡す形に
した（列挙ではないので、ゲームがつまみを増やしてもここは無編集）。

確かめ方は `bash -x docker/run.sh` の最後の行:

```
GARDEN_SELFTEST=1 GARDEN_RELOAD_AT=20 GARDEN_S8_TOUCH=1 SABIBOTS_SELFTEST=1 bash -x docker/run.sh garden release --headless 1
→ … -v rubevy-games-target-rubevy_games-wt-shared:/target …
    -e GARDEN_RELOAD_AT -e GARDEN_S8_TOUCH -e GARDEN_SELFTEST … /target/release/garden --headless 1
```

`GARDEN_` の 3 つが渡り、隣のゲームの `SABIBOTS_SELFTEST` は渡らない。

---

## 2. 著者が決めた既定値 — 生き物の VM の予算（A）

### 端数をどう扱うか

著者の指定は `1.74 × 23,686` ≈ **41,000**。式どおりなら **41,214** である。
**1,000 の位に丸めた**。理由は後から作った理屈ではなく、**同じ式の先例がこの repo にある**ことだ:
世界の VM の 45,000 は `1.74 × 25,837 = 44,956` を同じように丸めた数である
（`WORLD_BUDGET` の rustdoc）。同じ式を同じように丸めた。

丸めた結果、余裕の比は `41,000 ÷ 23,686 = 1.73` になる（1.74 ではない）。
これも rustdoc に書いた——「1.74 倍だ」と書いて 1.73 の数を置くのは、数と文が食い違う例そのもの
（`docs/numbers.md` §7-5 の `CLICK_REACH` が同じ形で残っている）。

### 測り直した — 23,686 は今 23,912 である

出どころの 23,686 は S5b-3 の測定で、それは **S5b-4 の前**の庭である。書く前に同じ席に座り直した:
`garden.settings.txt` に S5b-3 と同じ 3 行（`start_plants=130`・`start_beetles=14`・
`start_rabbits=10` = `world.rb` の `pop_max` 24）を置き、`--headless 60` を前後**交互に 3 巡**、
合わせて 6 走行 21,558 フレーム。仕掛けは S5b-3 と同じで、`ScriptWorld::last_frame()`
（rubevy の `FrameStats`）を毎フレーム 1 行印字する `GARDEN_MEASURE=1` のシステム 1 本。
**コミットには入れていない**（測った後に外した。S5b-2・S5b-3 と同じ扱い）。

| | 前（200,000） | 後（41,000） |
|---|---|---|
| フレーム数（3 走行） | 10,781 | 10,777 |
| 上限の庭（株 90 以上・24 匹）のフレーム | 1,533 | 1,439 |
| 生き物 VM の命令数 中央値（上限の庭） | 362 | 312 |
| 同 99 パーセンタイル（上限の庭） | 4,036 | 4,505 |
| **全フレームの最大** | **23,912** | **23,912** |
| 最初のフレーム（f0）の命令数 | 23,912（3 走行とも） | 23,912（3 走行とも） |
| f60 以降の最大 | 4,865 | 5,926 |
| **持ち越し `carried_reflect` / `carried_in_tick` の最大** | **0 / 0** | **0 / 0** |

読み取れること:

* **起動フレームは割れていない。** 予算を 5 分の 1 にしても、最初のフレームは前と**同じ
  23,912 命令**を使い切っている。予算に張り付いた（= 41,000 で切られた）フレームは 1 つも無く、
  持ち越しも 21,558 フレーム全部で 0。案 B（8,600）が「起動が 3 フレームに割れる」と
  予想されていたのに対し、案 A は割らない——これが著者の選んだ案の実物である。
* **23,686 ではなく 23,912。** S5b-3 の測定より 226 命令多い。差の出どころははっきりしていて、
  **S5b-4 が `beetle.rb` に足した `mutation_rate 0.1` の 1 行**である（`bef577e`）。甲虫 14 匹が
  最初の 1 パスで読む行が 1 つ増えた。226 ÷ 14 ≈ 16 命令/匹。
  つまり**実際に出荷される余裕の比は 1.74 でも 1.73 でもなく `41,000 ÷ 23,912 = 1.71`** である。
  rustdoc と一覧にそう書いた。**数は著者の 41,000 のままにした**——ここで 3 つ目の数
  （`1.74 × 23,912 → 42,000`）を勝手に作らない。著者への報告に回す。
* 定常の最大が 4,865 → 5,926 と動いているのは予算とは関係が無い（庭の乱数。`Dice` が時計から
  種を取るので同じバイナリでも 2 回同じ走行にならない）。どちらも 41,000 の 15% 以下である。

### 一緒に動いたもの

`window::scheduler_frames` は `2 + ceil(script_budget ÷ 45,600)` を**起動時に計算する**形に
S5b-3 がしてある。予算が動いたので**式を 1 文字も触らずに 7 → 3 フレーム**になった。
S5b-3 がこの形にしていなければ、判定は 41,000 の予算の VM を 200,000 の忍耐で待っていたことになる。
単体テスト `a_bigger_budget_buys_the_checks_more_frames` の 7 を 3 に直した（それが仕事の全部）。

`frame_time` は据え置き（著者の指定どおり）。上限の庭で生き物 VM の tick の最悪は 1.72 ms、
2 本合わせて 1 フレームの 30% で、絞る材料が無い。

### 前後で同じであることの確認

| 走行 | 前 | 後 |
|---|---|---|
| ヘッドレス `--headless 90`（交互 2 巡） | 13 行 FAIL 0 | 13 行 FAIL 0、`tools/fixedlines.sh` の diff 空 |
| 窓（docker、交互 2 巡） | 44 行 FAIL 0 | 44 行 FAIL 0、diff 空 |

窓の走行の「待った」の行は前後とも 1〜2 フレームで、新しい上限 3 に収まっている
（前の上限は 7 だった。**上限に近づいたのではなく、上限が実態に寄った**）。

### Battle の `script_budget` は動かしていない

Battle の 200,000 は rubevy の既定のままである。**測っていない**からで、箱庭の数を持ってくる
理由も無い（試合は機体 2 台で上限がはっきりしているが、`each_frame` の中身も `matches/*.rb` の
作りも箱庭とは別物である）。`docs/numbers.md` §4.5 に「箱庭は測って決めた、Battle は未測定」と
書いた。

---

## 3. 一時停止の 2 行 — `Vec` の `==` をやめた（B-1）

S8 が割った原因そのもの: `places() == test.places` は `Vec` 同士の比較なので、**値だけでなく
`Query` が生き物を返す順番まで比べていた**。その順番は archetype ごとなので、止まりの前後に
1 体が別の archetype に移る（rubevy が `ScriptTask` を付ける／`dress_animations` が `Animated` を
付ける／止める直前の `Eating` が着地する）だけで、動いていない世界が動いて見える。

直し方は本体が決めた S8 の (A)1 + (B)1 のとおり:

* **entity を鍵に引き当てて比べる**（`what_changed`。`Changes { gone, fresh, moved }`）。
* **「消えた／増えた／値が変わった」を別々に言う。** 判定の行は 2 → 3 行になった:
  `the same creatures are there`（顔ぶれ）、`every creature is where it was`（場所）、
  `nobody got hungrier`（メーター）。FAIL のときは誰がどう動いたかを文が名指しする。
* **許容は置かない。** 等しいことがこの判定の意味で、ここに閾値を置けば出どころの無い数が 1 つ
  増えるだけである。
* コメントに 1 行: **`P` は世界の規則と時計と 2 本の VM を止めるもので、Bevy の世界を凍らせる
  ものではない。** 止まりの中で `ScriptTask` や `Animated` が付くのは世界が動いたのではない。
* 一時停止の作り（`Paused`、`is_still`）は**変えていない**。順序の辺も足していない（S7 が
  `.chain()` に 1 本足して別の判定を割った前例がある）。

**world の VM も見るようにした**（S8 の気づき §206）。`P` は 2 本の VM の予算を一緒に 0 にするのに、
判定は生き物の VM しか見ていなかった。`window_selftest` は Bevy の引数 15 個だったので、
2 本を `BothVms`（`SystemParam`）にまとめて 15 のままにしてある（`FakePointer` と同じ手）。
行の文面が 2 つ変わった: `P pauses: both VMs' budgets are 0`、`P again gives both budgets back`。

### 仕込みで前後を確かめた

S8 の計器を今の木に当て直した（`GARDEN_S8_TOUCH=<n>`: 止まりの n フレーム目に、生き物 1 体へ
**空の marker component を 1 つ付けるだけ**。値は 1 つも変えない）。**常設していない**——
測った後に外した。

| 仕込み `GARDEN_S8_TOUCH=3` の窓の走行 | 直す前（`1ef1fd3`） | 直した後 |
|---|---|---|
| 1 巡目 | **FAIL 2**（`every creature is where it was`、`nobody got hungrier`） | ok 0 FAIL |
| 2 巡目 | **FAIL 2**（同じ 2 行） | ok 0 FAIL |
| 3 巡目 | **FAIL 2**（同じ 2 行） | ok 0 FAIL |

前後とも docker の window で、交互に走らせた。**3/3 で再現し、3/3 で直っている。**

仕込み無しの窓の走行は **45 行 FAIL 0**（前は 44 行）。`tools/fixedlines.sh` の diff は
**狙った 3 行の増減だけ**:

```
< selftest: ok   P again gives the budget back
< selftest: ok   P pauses: the scripts' budget is N
---
> selftest: ok   N s paused: the same creatures are there
> selftest: ok   P again gives both budgets back
> selftest: ok   P pauses: both VMs' budgets are N
```

`docs/verification/selftest-lines.md` の箱庭の窓（44 → 45 行）とブラウザ（45 → 46 行）を直した。

---

## 4. 残っていた 0.6 秒の待ち 2 か所（B-2）

S7 は割れた 4 つの待ちを条件待ちに変え、`world.rb` の Apply（`Ctrl+Enter`）とその Revert の
**2 か所を 0.6 秒のまま残した**——「同じ形だが、まだ落ちたことが無い」という理由で。同じ形なら
同じ直し方である。

待つものは `Turn::TheDayIs(f32)`: **規則が `garden.rules(day_length:)` で game に渡す数**が
その値になったこと。境界を数として越える唯一の規則で、起動したばかりの世界スクリプトしか
言わないので、「この規則が走っている」と 1 つの等式で言える（判定の文もそう書いてある）。

`Turn` から `Eq` の derive を外した（`f32` を持つため）。比較は `!= Turn::NotWaiting` の 1 か所
だけなので `PartialEq` で足りる。

### 実物が「上限ちょうど」を見せた — `spare` を足した

条件待ちにして最初に走らせたら、`Ctrl+Enter` の待ちが **ちょうど 3 フレーム**かかった。
上限も 3（`scheduler_frames`）である。**余裕が無い。**

数え直すと理由がはっきりした。`scheduler_frames` は「VM に何かを頼んだ瞬間から」の勘定である:
`Commands` で `Script` が付くフレーム + タスクが作られて走るフレーム + 予算 1 フレーム分。
ところがこの段は**キーで頼んでいる**ので、その前に 1 フレームある——`rubevy-egui` のパネルは
`Ctrl+Enter` を egui のパスで読むので、`Editor::action` が立つのは押した次のフレームである。
これは `Turn::EguiHasThePointer` が丸ごと扱っているのと同じ 1 フレームで、**待ちの中ではなく
待ちの前**にあるぶんだけ勘定から漏れていた。

`WindowTest::spare`（この待ちが VM の勘定の上に要するフレーム数。0 か 1 しか取らず、1 はその
egui のフレーム）を足し、上限を `frames + spare` にした。`Ctrl+Enter` の段だけ 1 である
（Revert は `Editor::action` を直接立てるので 0）。**新しく決めた数ではない**——
`EguiHasThePointer` の rustdoc が前から書いている構造のフレームを、もう 1 か所で数えただけ。

| 窓の走行 | 待ちのフレーム数（7 つの待ち） | 行 | FAIL |
|---|---|---|---|
| 1 巡目 | 1, 2, 2, **3**, **2**, 1, 1 | 45 | 0 |
| 2 巡目 | 1, 2, 2, **3**, **2**, 1, 1 | 45 | 0 |
| 3 巡目 | 1, 2, 2, **3**, **2**, 1, 1 | 45 | 0 |

太字が新しい 2 つ（`Ctrl+Enter` の 3、Revert の 2）。**行の集合は B-1 と同じ**——
待ちの直しは判定の文を変えない（「待った」の行は `tools/fixedlines.sh` が落とす stage direction）。

これで `window.rs` に残る「秒」は**秒を意味しているものだけ**になった: 止まりの 2 秒、歩きの
0.5 秒、VM が走り続けていることの 9 秒、そして Bevy の入力が 1 フレーム見るための 0.2 秒が 6 つ。

---

## 5. 判定が持っていた「実値の写し」（B-4・B-8）と、`--` と `n/a`（B-7）

S5b-2 が Battle で 1 つ見つけた形——**判定の閾値が、ゲームの側の値を書き写した数になっている**
——を、同じ目で箱庭に当てた。2 つ見つかった。どちらも「その数を直した人が、直したせいで
判定に落とされる」形である。

### (1) 「夜が来た」の 60 秒

`night arrived by 60 s` の 60.0 は直書きで、一覧（§2.11）には**「導出: `DAY_LENGTH` 1 周」**と
書いてあった。導出ではなく**写し**である: 1 日の長さは今 `world.rb` の `day_length` が言い、
`garden.rules` で `Sky::day_length` に届く。`world.rb` の日を 90 秒に伸ばした人は、規則が
守られたのに FAIL を見ることになる。実値（`sky.day_length`）を読む形にした。文面も
`night arrived within a day of N s` に変わる（N を印字するので、何と比べたかが走行に残る）。

### (2) 窓の「規則」の段が持っていた `60.0` と `"day_length 60.0"`

こちらは 2 か所あって、2 つ目の方が悪い:

* `ok(sky.day_length == 60.0, "the day is what world.rb says it is")` ——
  **文が「`world.rb` が言うとおりか」と言っているのに、比べていたのは書き写した 60.0** である。
* `editor.text.replace("day_length 60.0", "day_length 30.0")` ——
  日を変えた `world.rb` では**置換が何にも当たらず、黙って元の文のまま Apply される**。
  そのあとの `sky.day_length == 30.0` が落ちて、「Apply が効かない」という嘘の FAIL が出る。

`day_length_in(&editor.text)` でテキストから読み、その**半分**を書き戻す形にした
（半分である必要は無い——「違う数なら何でもよい」もので、ログで元の数と並べて読みやすいので
半分にした、と書いてある）。単体テスト 1 本を足した: 出荷される `world.rb` から 60.0 が読め、
書き戻した行が実際に置換に当たり、置換後は 30.0 が読める。

ほかに写しが無いかは §2.11 の 22 行を 1 つずつ見た。**残りは写しではない**:
`REACH + 0.5` は実値を読んでいる、`MUTATION_RATE` は S5b-4 が VM から読む形にした、
空腹バーの 55.0 と `hungry_below` 55.0 は「同じ値だが繋がない」と著者が決めた（§7-6）、
仕込みの空腹 45.0 は `START_HUNGER` の下端と同じ数だが別のこと（庭の初期値と仕掛けの値）で、
どちらの記録にも繋いだ形跡が無い——**一覧に「同じ値だが繋がっていない」と書くに留めた**。

### `INSTRUCTIONS_A_FRAME_BUYS` はどちらの実測を採るか（B-4）

実測は 2 つある。S6 は `frame_time` を **300 µs に絞って** 5.7 命令/µs（8 ms で 45,600）、
S5b-3 は上限の庭を**本来の 8 ms で**回して 6.85 命令/µs（54,800）。差は機械ではなく条件で、
300 µs のフレームは tick の出入りの費用を 27 分の 1 の仕事で払うので「短いフレームの速さ」になる。

**遅い方（45,600）を採った。** 理由は誤差の向きである: この数は予算を割って「何フレーム待って
よいか」を出すので、**大きすぎると待ちが短くなり、遅いだけの VM に FAIL を出す**——S6 と S7 が
消しに来た偽の FAIL そのもの。小さすぎれば判定が少し遅れるだけである。rustdoc に 2 つの条件を
表で書いた。

**実物で確かめた**: 既定の予算 41,000 では `ceil(41,000 ÷ 45,600)` も `ceil(41,000 ÷ 54,800)` も 1 で、
`scheduler_frames` は**どちらでも 3**。差が出るのは `script_budget` が 45,600 を超えたときだけである。

### `--` と `n/a`（B-7）

意味は同じ「測れなかった」。Battle は `--`、箱庭は 1 か所だけ `n/a` で、残りは `--` だった。
**`--` に揃えた**（多数派であり、2 つのゲームが同じ綴りになる）。`tools/fixedlines.sh` の
verdict の正規表現から `n/a ` を外し、`docs/verification/selftest-lines.md` の verdict 表と
`docs/garden.md` の 4 か所を直した。6 通りの走行では `n/a` の行は 1 つも出ていないので、
記録された一覧の行は変わらない。

---

## 6. ホイールの 5 つ目の揺れ（B-3）と、狙う点（B-5）

### 原因を実物で確かめた

S7 は「egui の『ポインタを持っている』が揺れる」と書き、`held` をホイールを回した瞬間の値に
するという案を残していた。**確かめてから直した。** 計器は 1 行で、判定の直前に
「ホイールを回したフレームの `held`」と「判定のフレームの `held`」を並べて印字するもの
（常設していない）。**8 本同時**の走行を交互に回すと、直す前の失敗した走行にこう出た:

```
selftest: probe wheel-frame held=false judge-frame held=true
selftest: FAIL and with the panel closed the same wheel in the same place zooms
          (egui holds the pointer: true; camera 42.00 -> 38.18)
```

**カメラは 42.00 → 38.18 とちゃんとズームしている**——つまりホイールを回した瞬間、egui は
ポインタを持っていなかった。判定はその 0.2 秒後に読んで「持っている」と言い、正しい動作を
FAIL と呼んでいた。S7 の見立てのとおりである。

直しは `WindowTest::held`: ホイールを回すフレームで `egui_has_it()` を控え、判定はそれを使う。
2 つのホイールの判定の**両方**に入れた（形が同じで、片方だけ直す理由が無い）。

### 直さなかったら分からなかったこと — 待ちの上限

直した版でも同じ行が落ちる走行があり、中身が違った:

```
selftest: waited 3 frame(s) for the thing the next check is about, and it did not — judging it as it stands
selftest: FAIL … (egui holds the pointer: true; camera 42.00 -> 42.00)
```

**待ちが上限で諦めている。** `Turn::EguiHasThePointer(false)`（エディタを閉じたので egui が
ポインタを放したこと）を待つ上限は `scheduler_frames` で、**A で予算が 5 分の 1 になったせいで
7 → 3 フレームになった**。egui の待ちは VM の待ちではないのに、S7 が「3 つのうち最大の 1 つを
使う」と決めた数に相乗りしている。予算が縮んだら、相乗りしていた方が先に足りなくなった。

ここで新しい上限の数を作らなかった（作れる出どころが無い）。代わりに**前提を先に確かめる**形に
した: この段はエディタを閉じてから、エディタが在った場所でホイールを回す。**ゲームは途中で
エディタを開け直すことがある**——`choose_watched` は見ている生き物が死ぬとエディタを開く——ので、
ホイールを回したフレームの `editor.open` を控え、それが真なら **`--`（測れなかった）**と言う。
ok でも FAIL でもない、という第 3 の verdict は箱庭が前から持っている。
FAIL の走行には、そのフレームの `wants_pointer_input` と `is_pointer_over_area` を分けて
印字する stage direction を 1 行足した（次に落ちた人が同じ測り直しをしないため）。

### 狙う点（B-5）

`over_the_editor()` は矩形の**真ん中**を狙っていた。パネルは窓の右端に張り付いているので、
**幅の設定が効いていなくても真ん中は必ずパネルの中**である（S5b-1 の気づき。著者は
`editor_width=1400` を手で置いて確かめた——判定が直してくれと言っている形）。

矩形の**内側の辺**（幅が変わると動く唯一の辺）から `margin` だけ入った点にした。設定より
狭いパネルはそこに届かない。**窓の右半分に留める**制限を 1 つ付けた: 左には HUD（上）と VM
パネル（下）があり、control の段は「そこに何も描かれていない」ことが前提だからである。
制限が効くのはエディタが窓の半分より広いときだけで、そのときは幅についてもう言えることは
言っている。

---

## 7. ブラウザから計測のつまみを渡す口（C-1）

S8 は「ブラウザでだけ出る揺れを、ブラウザで追えない」と書いて止まっている: 判定のつまみは
`GARDEN_RELOAD_AT` のような環境変数で、wasm に `std::env` が無いので `asked_number` は
ページでは常に `None` を返していた。**計器を点けられない場所でしか起きないものは測れない。**

`games_shell::checks::asked_number` がページでは**アドレス**を読むようにした:
PC の `<GAME>_<NAME>=<value>` が、ページでは `?selftest&<name>=<value>` である。
綴りを繋ぐ規則は `query_name` 1 か所——環境変数名の最初の `_` までを落として小文字にする
（`GARDEN_RELOAD_AT` → `reload_at`）。接頭辞は「1 つのシェルで 2 つのゲームの変数を分ける」
ためのもので、ページにはそのシェルが無いから落とす。単体テスト 1 本（ゲーム名が 2 語のときは
ここに書き足す必要がある、とコメントに書いた。今はどちらも 1 語）。

PC 側は無変更で、`docker/run.sh` が `<GAME>_` の変数を全部渡すようになった（§1）ので、
**同じ名前のつまみが 3 つの走らせ方（PC・コンテナ・ページ）で通る**ようになった。

---

## 8. 種の遺伝子は「庭の家具」になった（D-1）

S5b-4 が移せなかった数——`Genome::of` の 2 × 3 と `SPREAD` 0.18——について、著者の判断は
**案 A: `Furniture` の設定へ**（分類 (b) → (c)）。鍵は 7 つ:
`start_beetle_speed` / `_sight` / `_appetite`、`start_rabbit_speed` / `_sight` / `_appetite`、
`start_genome_spread`。**既定値は 1 つも動かしていない。**

`world.rb` に移すと嘘になる、という S5b-4 の見立てはそのまま正しい: この 3 行を読むのは
`spawn_world`（`Startup`）だけで、`garden.rules` が届くのは最初の `Update` である。
**庭を建てるときに 1 度だけ読む数は、株の数・個体の数・最初の空腹と同じ「最初の家具」**であって、
規則ではない。`Furniture` に入れると分類の文が実物と合う。

作りは小さい:

* `genome::BEETLE` / `genome::RABBIT` / `genome::SPREAD` が既定値の名前になり（`pub const`）、
  `Genome::of` はその 2 つを返すだけになった。**`Genome::of` は消していない**——
  判定の仕込み（断食甲虫・プローブ）と、走行の終わりに「この庭の平均遺伝子」を種の元の数と
  並べて印字する行は、*庭*ではなく*種*の話だからである。
* `Genome::roll` は `(species, jitter)` ではなく `(base, spread, jitter)` を取る。
  基準も散らばりも設定なので、種から引くのをやめて呼び手が渡す。
* `Furniture::genome(species)` が庭の答え。`spawn_world` が使う唯一の口である。

出どころも 1 行ずつ書いた: speed と sight は **不明**（G1 が書いた数で、記録が無い）、
appetite 1.0 は **導出**（`world.rb` の `hunger_rate` に掛かるので、1 は「G1 が走らせた世界
そのもの」）、`SPREAD` 0.18 は *理由のみ*（「甲虫が甲虫でなくなるほどではなく、2 親に平均する
差はある」）で数そのものは不明。

単体テストは既にある設定のテストに 4 行足した: 書いた鍵は動き、書かない鍵は種の既定のまま、
`start_genome_spread=0` の庭では `roll` が基準そのものを返す（`jitter` が `(1.0, 1.0)` で
呼ばれることまで見ている）。

---

## 9. 揺れの前後 — 8 本同時・交互 11 巡・33 分

S6 と S7 と同じ条件（窓の走行を **8 本同時**、docker、交互に A/B）。
前は `21040c1`（§4 まで入った版）、後は `35158bb` の `held` の直しを当てた版で、
**別の worktree・別の docker volume・別の `/target`**（§1 でそうなった）。
`md5sum` で別のバイナリであることを確かめてある。機械の `vmstat` の idle は各巡の終わりに
記録して **73%（1 巡）・96%（1 巡）・99〜100%（20 巡）** ——ほぼ空いていた。
07:12 から 07:45 まで、**各側 88 走行**。

| | 直す前 | 直した後 |
|---|---|---|
| FAIL の出た走行 | **6 / 88** | **3 / 88** |
| うち「判定が `held` を遅れて読んだ」（`wheel-frame held=false judge-frame held=true`） | **3** | **0** |
| うち「egui の待ちが上限で諦めた」（`waited 3 … did not`） | 3 | 3 |
| 行の集合 | 45 行（全走行） | 45 行（全走行） |

**狙った揺れは 3/88 → 0/88 になった。** 残る 3 件は `held` とは別の形で、前後で同じ数である
（直しはそこに触っていないので、当然そうなる）。その 3 件に対する手当（前提を先に確かめて
`--` と言う）は**この測定の後にコミットした**ので、上の表には入っていない——下の §11 で
最終版をもう一度同じ条件に掛ける。

S7 の記録（8 本同時 88 走行で `and with the panel closed …` が 4）と比べると、直す前の 6 件は
その 4 件と同じ行で、内訳が 2 つに分かれていたことが今回分かった、という関係になる。

---

## 10. 狙う点を動かしたら、下に何が居るかが分かった

B-5 で狙う点を矩形の真ん中（既定で x=1332）から内側の辺（x=1080）へ動かしたら、
**窓の走行が 3/3 で落ちた**。前の版では出ていない形である。

足しておいた stage direction が 1 発で答えを出した:

```
selftest: in the frame the wheel was turned: wants_pointer_input=true is_pointer_over_area=true editor.open=false
selftest: FAIL and with the panel closed the same wheel in the same place zooms (… camera 42.00 -> 42.00)
```

**エディタは閉じている（`editor.open=false`）のに、egui は「何かの上にポインタが居る」と言う。**
その何かは**説明のパネル**だった: `games_shell::Guide` は窓の中央に錨を下ろし、
`Order::Foreground`（他の全パネルの上）で描かれ、**ゲームが始まった時点で開いている**。
既定の大きさ 640×820 を 1600×900 の窓の中央に置くと x は 480〜1120 で、
**新しい狙う点 1080 はその中**、古い狙う点 1332 は外である。

つまり 2 つのホイールの判定は、**「狙う点が説明パネルの外に落ちる」ことに黙って寄りかかって
いた**。150 px の余裕で。狙う点を動かすまで誰も知らなかったし、
`guide_width` を設定で広げた人は同じ FAIL を見たはずである。

直しは `FakePointer::hide_the_guide()`——ホイールの段に入る前に `Guide::open` を false にする。
`H` を打つのではなく代入にしたのは、**欲しいのは「閉じていること」であって「切り替わること」
ではない**から（同じ段が `Editor::open` を既に代入で扱っている）。`FakePointer` は
`SystemParam` なので `window_selftest` の引数は 15 のままである。

直した後の窓の走行は **3/3 で 45 行 FAIL 0**。

---

## 11. `docs/web.md` の表と、`WorldMeter::passes` の名前（D-3）

### wasm の大きさ

S5b-1〜4 の 4 段階で 4 回ずれたので、**4 つの列を同じ機械・同じ数分で取り直した**
（`--profile web`、binaryen 132）。取り方は前と同じで、`wasm-opt` の前の数は
`wasm-bindgen` を一時ディレクトリへ出して測った。

| | bytes | gzip -9 | `wasm-opt -Os` | その gzip |
|---|---|---|---|---|
| `sabibots` | 39,212,650 | 10,060,181 | 35,216,606 | 10,768,420 |
| `garden` | 40,035,901 | 10,283,236 | 35,980,029 | 10,992,359 |

09-20 の同じ 4 列に対して Battle は **+111,915（+0.29%）／+104,988（+0.30%）**、
箱庭は **+162,350（+0.41%）／+151,729（+0.42%)**。中身は S5b そのもの（箱庭の 7 つの
Resource と Battle の 1 つ、それを読む `Settings` の鍵、Ruby 側の模型、判定の新しい行）。

### `WorldMeter::passes` → `ran_in`

S7 の気づき: **名前に反して「スクリプトが 1 命令でも走ったフレーム」を数えている。**
出荷される `world.rb` では両者は同じ数になる——`Rubevy.ask("frame")` が 1 フレームを食う唯一の
問いなので、1 フレーム = `each_frame` 1 パス（90 秒で 5,373 フレーム中 5,372 パス）。
だがそれは**その規則についての事実**であって、測り方についての事実ではない。
フレームを食う問いを 2 つ聞く `world.rb` でも、パスが 1 フレームで終わらない `world.rb` でも、
この `Vec` は「走ったフレーム」を 1 つ積む。

名前を `ran_in` にし、印字も直した:

* 走行の終わりの行: `{} passes of \`each_frame\`` → `{} frames they ran in`
* 12 番目の判定の文: `over N passes of \`each_frame\`` → `over N frames they ran in`

**HUD の文言は変えていない**ので、`guide_text.rs` もフォントの再サブセット
（`tools/subset-font.sh`）も要らない——確かめた。HUD が描く「規則」の行は
`the rules {last_pass} / {budget} insn/frame · {ms}` で、`last_pass` は
「このフレームで規則が使った命令数」であり、名前も文も元から正しい。
hover の文（`what one pass of ruby/world.rb's each_frame cost this frame`）も
出荷される規則については正しいままである。

判定の文が 1 つ変わるので `docs/verification/selftest-lines.md` と `docs/garden.md` の
走行例を直した。ヘッドレスの 13 行は取り直して一致。

---

## 12. 最後の網 — 一覧に無かった数（D-2）

S5a の網は `const` と名前のある数に寄っていた。今度は**数値リテラルを含む行を機械的に拾い直した**。
拾い方は 3 段で絞った:

1. 数値リテラルを含む行を全部（箱庭 428・全体 1,030 行）。
2. **調整値の形をしたもの**だけ（小数、または 4 以上の整数）——添字とループの 0〜3 を落とす。
   箱庭 335・Battle 209・共有 crate 128 行。
3. **関数の本体に直接書かれたもの**だけ——`const` 宣言と `impl Default` の中は「既定値に名前が
   付いているもの」で、一覧が既に持っている。箱庭 220・Battle 191・共有 crate 70 行。

この 3 段目を目で通した。**S5b-2 と S5b-3 が名指しした抜けは全部そこに居た**（昼の色の傾き、
空の勾配の押し上げ、最初の草の下限 0.3、Battle の弾・砲塔・名札・撃破機の灰色・z 座標）。
新しく見つかったものもある: 昼の太陽の色 3 つ、夜の月の色、環境光の色 2 組、空の板の色、
記録板の 6 色、影のずらし 0.12、爆発の褪せ。

**`docs/numbers.md` §2.14 に 14 行の表として足し、分類案と出どころ（ほとんど「不明」）を書いた。
移していない。** 理由は 2 つ: この段階は判定の側 (d) の仕事で、移すのは範囲の外であること。
そして **1 件ずつ「誰がいつ読むか」を見ないと移し先が決まらない**こと——S5b-4 が遺伝子で学んだ
ことで、`Startup` でしか読まれない数を `world.rb` に置けば「効かない設定」になる。
z 座標一式は分類が (c) ですらない: **数より重なりの順序が不変量**で、(a) に寄る。

§6 の合計には足していない。上の表の「1 行」は数 1 つではない（「昼の色の傾き一式」は 6 つの数）
ので、粒度を揃えて数え直すのは移すときの仕事である。そう書いた。

---

## 13. やらなかったこと — Save を安全に押す段（B-6）

S5b-3 の気づき「`save_file` の設定が実走行で確かめられていない。窓のチェックは
**リポジトリに書くので Save はわざと入れていない**」に対して、**設計は立てたが実装していない。**
理由は「この段階の終わりに新しい判定を足して、確かめる時間が足りない」であって、
できないからではない。次の段階への材料として置いておく。

立てた設計:

* 押すのは**庭のセーブ**（F5 / `Asked::save`）で、エディタの Save ではない。
  エディタの Save は `garden/ruby/*.rb` に書くので、行き先を変えられない。
* 行き先は `SaveFile::path` を判定が一時的に差し替える。PC は `std::env::temp_dir()` の下、
  ブラウザは `localStorage` の別の鍵——**`platform::read` / `platform::write` が両方を同じ 2 行で
  扱う**ので、判定の側は 1 つの道で書ける。
* 確かめ方は `platform::read(その道)` が JSON を返すこと（`"version"` を含むこと）と、
  `SaveNote::bad` が false であること。PC では読んだあと消す。
* **リポジトリには何も書かない。** ブラウザには `garden:` の鍵が 1 つ残るが、
  それは `?selftest` を自分で付けた人の browser だけである（判定が走る条件）。

障害が 2 つあり、どちらも越えられる:

1. `window_selftest` は Bevy の引数 15 個で、`SaveFile` / `Asked` / `SaveNote` の 3 つを足すと
   上限 16 を超える。**`SystemParam` 1 つにまとめる**のが答え（`FakePointer`・`HudLook`・
   `BothVms` と同じ手を 4 度目に使うことになる）。
2. 判定の行が 1〜2 行増えるので `docs/verification/selftest-lines.md` の**箱庭の窓とブラウザ**の
   一覧を直す必要がある。

---

## 14. 気づいた点（仕事の範囲の外。直していない）

1. **egui の待ちが VM の数に相乗りしている。** `Turn::EguiHasThePointer` は VM と何の関係も
   無いのに、上限は `scheduler_frames`（= `2 + ceil(script_budget ÷ 45,600)`）である。
   S7 は「3 つのうち最大の 1 つで足りる、1 つの数の方がよい」と決めたが、**A で予算が 5 分の 1 に
   なった結果、その「最大」が 7 → 3 フレームになり、egui の待ちが先に足りなくなった**
   （8 本同時で 3/88 の走行が諦めている）。egui 側の上限に出どころのある数を与えるか、
   S7 の「1 つの数」をやめるか。どこ: `garden/src/window.rs` の `scheduler_frames` と `Turn`。
   属する先: 箱庭の判定／計画（S7 の決定の見直し）。**本の素材にもなる**——
   「別々の理由の待ちに 1 つの数を使うと、片方の理由が動いたとき、もう片方が黙って壊れる」。

2. **著者の 41,000 の出どころ 23,686 は、S5b-4 の前の庭の数である。** 今日測り直すと 23,912 で、
   出荷される余裕は 1.74 でも（丸めた）1.73 でもなく **1.71 倍**。同じ式を今日の実測に当てると
   `1.74 × 23,912 → 42,000` になる。**数は著者のまま置いた**（勝手に 3 つ目の数を作らない）。
   属する先: 埋め込みの数／著者の判断。

3. **2 つのホイールの判定は、説明パネルの位置に黙って寄りかかっていた**（150 px の余裕で）。
   狙う点を動かすまで誰も知らなかった。`guide_width` を広げた人は同じ FAIL を見る。
   直しは入れた（`hide_the_guide`）が、**同じ形がほかにも無いかは見ていない**——
   「判定が、書かれていない配置に依っている」という穴の種類である。属する先: 本の素材／箱庭の判定。

4. **`window_selftest` は Bevy の引数 15 個**で、上限 16 まであと 1 つ。この段階だけで
   `SystemParam` にまとめる手を 3 度使った（`FakePointer`・`HudLook`・`BothVms`）。
   B-6（Save の段）はもう 1 度使うことになる。属する先: 箱庭の判定の作り。

5. **Battle の `script_budget` は rubevy の既定 200,000 のまま**で、測っていない。
   箱庭の両方の VM が「測って決めた数」になった今、この repo に残る唯一の「引き継いだ予算」である。
   属する先: 出どころの無い数／次の段階。

6. **`docs/numbers.md` §6 の合計と §2.14 の粒度が違う。** §2.14 の 1 行は数 1 つではない
   （「昼の色の傾き一式」は 6 つ）。移すときに数え直す、と書いた。属する先: 一覧の作り。

7. **`docs/verification/selftest-lines.md` の「verdict が動きうる行はちょうど 1 つ」が 2 つに
   なった**（ホイールの control が `--` を出しうる）。文は直した。属する先: 確認の作法。

8. **古い docker volume が置き去りになっている。** `rubevy-games-target`（共有だったもの）と、
   前の担当が手で作った `-s8` / `-s8-before` / `-leftovers` / `-garden1` / `-world`。
   それぞれ release の bevy 一式ぶんの大きさがある。消すかどうかは使う人の判断
   （§1 に「`docker volume rm` は使う人が決める」と書いた）。属する先: 道具／機械の掃除。

9. **隣の worktree の待ちのシェルが 2 つ残って回っている**（`until … sleep 20` のループ。
   前の担当のもの）。止めていない（他の担当のプロセスを止めない規則）。機械は 99% idle のままで、
   測定には影響していない。属する先: 作法。

---

## 15. 段階の終わりの確認

| 何を | 結果 |
|---|---|
| `cargo build --workspace --all-targets` | **警告 0・エラー 0** |
| `cargo test --workspace` | **60 通過**（着手前 58。足したのは `query_name` の 1 本と `day_length_in` の 1 本） |
| 箱庭ヘッドレス `--headless 90` | 13 行 FAIL 0、一覧と一致 |
| Battle ヘッドレス `--headless 25` | 4 行 FAIL 0、一覧と一致（`the handler tasks …` が `--` の走行。**verdict が動く行**として前から記録されている） |
| 箱庭 窓（docker） | **45 行** FAIL 0、一覧と一致（44 → 45） |
| Battle 窓（docker） | 32 行 FAIL 0、一覧と一致 |
| 箱庭 ブラウザ `?selftest` | **46 行** FAIL 0、pageerror 0、requestfailed 0（45 → 46） |
| Battle ブラウザ `?selftest` | 33 行 FAIL 0、pageerror 0、requestfailed 0 |
| `web/build.sh all` + `wasm-opt` | 通る（大きさは §11） |

### 揺れ — 最終版を同じ条件に掛けた

**8 本同時・6 巡・48 走行**（idle 99〜100%）:

| | FAIL の出た走行 | 行 | 待ちが上限で諦めた回数 |
|---|---|---|---|
| 直す前（`21040c1`、§9 の表の左） | 6 / 88 | 45 | 3 |
| `held` だけ直した版（§9 の表の右） | 3 / 88 | 45 | 3 |
| **最終版**（説明パネルを閉じる・前提を確かめる） | **0 / 48** | 45 | **0** |

**待ちが一度も上限に達していない。** §9 で残っていた 3 件の正体は、直接には確かめていないが、
説明パネルを閉じたら消えた——ホイールを回す前に egui が何かを掴んでいる、という同じ形である。
§14-1 に書いたとおり、**egui の待ちが VM の数に相乗りしている**という構造の問題は残っている
（今は上限に当たらないだけで、`script_budget` をもっと下げれば当たる）。

### 行の集合が変わったところ（`docs/verification/selftest-lines.md` を更新した）

| 行 | 何が起きたか |
|---|---|
| `P pauses: the scripts' budget is N` → `P pauses: both VMs' budgets are N` | 判定が 2 本の VM を見るようになった（B-1） |
| `P again gives the budget back` → `P again gives both budgets back` | 同上 |
| **`N s paused: the same creatures are there`（新）** | 「消えた／増えた」を場所と空腹から分けて言う（B-1） |
| `night arrived by N s` → `night arrived within a day of N s` | 閾値が実値になった（B-8） |
| `the day is what world.rb says it is` → `… (N s)` | 読んだ値を印字する（B-8） |
| `… over N passes of \`each_frame\`` → `… over N frames they ran in` | 名前が実物に合った（D-3） |

箱庭の窓 44 → **45 行**、ブラウザ 45 → **46 行**。Battle の 3 つは無変更。
