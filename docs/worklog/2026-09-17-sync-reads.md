...
# 9. 追記: 判定の測り方を仕様に合わせた — 「0.5 秒後の 1 点」から「0.5 秒の窓」へ

§8 の報告を受けた本体の判断: **`flee_from` は触らない。原因 A・B とも判定の測り方の問題なので
`watch_turning` を直す。** 6 つ目の判定を「0.5 秒後の 1 点の向き」から
「**0.5 秒の窓の間に一度でも向きが変わったか**」にする。閾値（0.5 秒、`dot < 0.7`）は動かさない。

## 9.1 これは閾値の変更ではなく、文言と測り方を合わせる変更

判定の名前は最初から `a beetle touched by a rabbit **changed heading within 0.5 s**` だった。
「within 0.5 s」は**窓**の言い方で、「0.5 秒後の 1 点」ではない。
G4 が 1 点で測ったのは、それで足りていたからで、足りなくなった理由が §8 の A と B:

* **A（窓の中の 2 通目）**: 1 通目のハンドラが 99° 回したあと、2 通目が「新しい進路」から swerve を選んで
  向け直す。1 点で見ると、判定が覚えている `was` からは 38° しか回っていない。
  **2 つのハンドラはどちらも仕事をしている。**「起きた転回が、見る前に上書きされた」だけ。
* **B（`sleep 0.5` 明け）**: ハンドラが舵を握るのは 0.5 秒、判定がのぞくのは 0.502 秒後。
  **同じ 0.5 が両側にある。** 同期読みで反射が 1 フレーム早まったぶん、`drop_wheel` 後の
  脳の `act` が窓の中に入る。

どちらも「転回は窓の中で起きていた」。**窓の中を毎フレーム見れば、どちらも見える。**
だから直すのは判定の測り方で、閾値でも、ハンドラでも、`flee_from` でもない。

`0.5` も `0.7` も 1 文字も動かしていない。「転回した」の式も同じ:
`b == Vec2::ZERO || a.dot(b) < 0.7`——**同じ式を、1 回ではなく窓の各フレームで評価する**だけ。
除外（静止・壁際・`NEWBORN_GRACE`・`TOUCH_SETTLE`）もそのまま。

## 9.2 作ったもの

`test.touched: Vec<(Entity, f32, Vec2)>` を `Vec<Touch>` にした:

```rust
struct Touch {
    beetle: Entity,
    at: f32,        // publish された時刻
    was: Vec2,      // そのときの進路
    turned: bool,   // 窓の中で一度でも 1/4 回転を超えたか
    closest: f32,   // 一番近づいた dot と、その時刻（miss 行の証拠）
    closest_at: f32,
    looked: u32,    // 窓の中で「読めた」フレーム数（壁際・消滅は読めない）
    last: Vec2,     // 最後に読めたときの速度（miss 行用）
}
```

`watch_turning` は毎フレーム、待っている `Touch` ごとに:

1. 消滅していたら落とす（今までどおり）。
2. **壁際ならそのフレームは飛ばす**（`move_creatures` が壁で速度成分を 0 にするので向きが読めない）。
   1 点で見ていたときの「壁際なら数えない」を、窓に翻訳するとこうなる。
   **窓の全フレームが壁際だった接触は数えない**（`looked == 0`）。
3. 読めたら `dot` を取り、`closest` を更新し、`dot < 0.7` なら `turned = true`。
4. 窓が閉じたら（`now - at >= 0.5`）数える。`turned` なら ok、でなければ miss 行。

miss 行は「一番近づいた dot とその時刻」「見たフレーム数」も出すようにした（指示 2）。
**一度も転回しなかった**という主張に大きさが付く。

## 9.3 結果 — 10 回で 6 つ目は 0 miss

| 回 | 6 つ目 | 数えた接触 | 外した | 他の判定 |
|---:|---|---:|---:|---|
| 1 | ok | 19 | 0 | **FAIL** 5 つ目「a hungry creature with a plant in sight reached it (got no closer than 2.0)」 |
| 2 | ok | 31 | 0 | ok |
| 3 | ok | 29 | 0 | ok |
| 4 | ok | 14 | 0 | ok |
| 5 | ok | 17 | 0 | ok |
| 6 | ok | 24 | 0 | **FAIL** 5 つ目（no closer than 2.2） |
| 7 | ok | 26 | 0 | ok |
| 8 | ok | 22 | 0 | ok |
| 9 | ok | 29 | 0 | **FAIL** 4 つ目「nothing walked through anything (closest pair 0.842 of the radii, 1 frames under 0.9)」 |
| 10 | ok | 31 | 0 | ok |
| 計 | **10/10 ok** | 242 | **0** | 他が落ちたのは 3 回 |

**6 つ目は 10 回とも通り、242 接触で外れ 0。** 直前（§8.3、1 点で見ていたとき）は
248 接触で外れ 7・4 回 FAIL だったので、A も B も消えた。
数えた接触の数（242 対 248）はほぼ同じで、**除外が増えて通ったのではない**ことが分かる。

## 9.4 代わりに見えてきたもの — 4 つ目と 5 つ目が落ちる

6 つ目が静かになったので、**別の 2 つの判定が時々落ちる**のが見えるようになった。
どちらも私は触っていない（指示 4「記録だけして触らない」）。

* **5 つ目「a hungry creature with a plant in sight reached it」** — 10 回中 2 回。
  通るときは**毎回ちょうど 1.78 秒**（8 回中 7 回が 1.78、1 回が 1.79）。
  落ちるときは「5.0 から 2.0 / 2.2 までしか近づかなかった」。
  **通るか、まったく行かないかの二択**で、途中が無い。仕込みの筋書き
  （空腹のビートルを 5.0 離れた草 1 本の前に置く）なので、最初の 1 パスが決めている形に見える。
* **4 つ目「nothing walked through anything」** — 10 回中 1 回。
  `closest pair 0.842 of the radii, 1 frames under 0.9`——**1 フレームだけ** 0.9 を割った。
  通る回は毎回 `0.974`。

順序を入れる前にこの 2 つが落ちるのを見たことが無かったので、順序を疑った。
**対照を取ったら、半分は外れだった**（§9.5）。

## 9.5 対照: 順序を入れていないビルドで 10 回

順序を入れる前のバイナリ（`cargo update` だけ、判定も 1 点のまま）を取ってあったので、
同じ機械で 10 回走らせた。結果は **10 回中 1 回、4 つ目が落ちた**:

```
ctrl 1: selftest: FAIL nothing walked through anything over 5365 frames (closest pair 0.859 of the radii, 1 frames under 0.9)
ctrl 2〜10: FAIL なし
```

これで両方の判定について、きれいに数えられる（instrument を入れて走らせた回は
実行時間が変わるので全部除いてある）:

| 判定 | 順序なし | 順序あり |
|---|---|---|
| 4 つ目「nothing walked through anything」 | **1 / 24** | **1 / 26** |
| 5 つ目「a hungry creature … reached it」 | **0 / 24** | **3 / 26** |

* **4 つ目（すり抜け）は順序と関係ない。** 順序の前も後も 1 回ずつで、どちらも
  「1 フレームだけ 0.9 を割った（0.842 / 0.859）」。**元から時々落ちる判定**で、
  今回それを見つけたというだけ。`separate` が `SEPARATE_PASSES` 回の反復で押し戻す形なので、
  速い 2 体が正面から入ると 1 フレーム食い込むことがある、という筋は通る。**触っていない。**
* **5 つ目（草に着く）は順序を入れてからしか見ていない。** 26 回で 3 回、その前は 24 回で 0 回。
  ただし Fisher の正確検定で両側 p ≈ 0.24 で、**これだけでは「順序のせい」と言えない**。
  言えるのは「順序を入れる前に 24 回見て 0 回、入れてから 26 回見て 3 回」まで。

5 つ目の落ち方には特徴がある。**通るときは毎回ちょうど 1.78 秒**（8 回中 7 回が 1.78、1 回が 1.79）で、
落ちるときは「5.0 から 2.0 / 2.2 までしか近づかなかった」。**途中が無い。**
仕込みの筋書き（空腹のビートルを 5.0 離れた草 1 本の前に置く）なので、
最初の 1 パスで `garden.nearest(:Plant)` が草を見つけるかどうかが全部を決めている形に見える。
順序を入れると `sprout_plants` と `eat` がティックの前に動くので、
最初のパスが見る世界が 1 フレームぶんずれる——という筋は立つが、**確かめていない**。
指示（「記録だけして触らない」）に従って、ここで止める。

## 9.6 やっていないこと

* `flee_from`、`0.5`、`dot < 0.7`、`TOUCH_SETTLE`、`NEWBORN_GRACE`、ハンドラの `sleep 0.5` —
  **1 文字も触っていない。**
* 4 つ目・5 つ目の判定も**触っていない**（指示 4）。
* miss 行は残した（指示 3）。窓になって出番は減ったが、次に落ちたときに
  「一番近づいた dot とその時刻」まで出るようになっている。

## 9.7 窓にして判定は甘くなったか

なった。正直に見積もる。窓のどれかのフレームが**ハンドラ以外の理由で** 1/4 回転を超えれば、
それも「転回した」と数える。ハンドラが火を噴かなかったビートルは脳の `wander` に任される:
`wander(change = 0.25)` は 1 パスにつき確率 0.25 で進路を振り直し、1 パスは `sleep 0.2` なので
0.5 秒に約 2.5 パス。少なくとも 1 回振り直す確率は約 51%、振り直した先が 45.6°（`dot 0.7`）より
外れる確率は約 75%。掛けて **1 接触あたり約 38%** が「ハンドラ抜きでも通ってしまう」確率。

これは**1 接触あたりの感度**であって判定の結論ではない。判定は 1 回の run の全接触
（20〜30 件）に対する `turned / checked` で、ハンドラが死んでいれば 6 割の接触が外れるので
FAIL になる。逆にハンドラが生きていれば、窓の間ずっと舵を握っている（`take_wheel` →
`sleep 0.5` → `drop_wheel`）ので、**転回させたものは他にいない**。
この見積もりは `watch_turning` の rustdoc にも書いた。
