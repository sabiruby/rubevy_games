...
# 2026-09-17 W3 — 窓・ブラウザ・説明・文書、そして予算の測り直し

同じ計画書（`docs/plans/garden-world-plan.md` 第 2 版）の **W3**。W2（`6f37d68`）の続きで、同じ worktree・
同じブランチ。ここまでで規則は全部 Ruby になったが、**それを触る道がまだ無い**——`world.rb` を書き換えるには
エディタを閉じてファイルを開くしかなく、「箱庭を止めずに規則を変える」は 12 番の判定の中にしか存在していなかった。
W3 はその道を作る段階で、作る量より**決める量**が多かった。

## 14. `F3` は何を切り替えるのか——「見ている生き物」と「開いているファイル」は別の事実

最初に決めたのはここで、実装より先に `window.rs` を読んで決めた。

今の窓は `Watched { entity }` 1 つで動いている。`show_code`（`window.rs:190`）は
「見ている生き物 → その `Mind.species` → その種のファイル」とたどってエディタを埋め、
`show_vm`（`window.rs:451`）は同じ entity の `ScriptTask` をパネルに出し、HUD はその行に `▸` を付ける。
**3 つのパネルが 1 つの事実から引かれている。**

`world.rb` を足すとその事実が 1 つでは足りなくなる。案は 2 つあった:

1. `Watched` を `enum { Creature(Entity), World }` にする。エディタから見ると素直だが、
   **VM パネルと HUD が行き場を失う**——世界を開いている間、パネルは何を出すのか。
   「世界の VM」を出す道は W3 の範囲ではないし（`VmInspector` は `ScriptWorld` を名前で取る）、
   「何も出さない」は、規則を読みながらウサギの様子を見る、という一番ありそうな使い方を殺す。
2. `Watched { entity, world: bool }` にする。**見ている生き物は見ている生き物のまま**で、
   エディタだけが別のファイルを映す。

**2 にした。** 理由は一言でいうと「パネルは 3 つあって、2 つ目のファイルを持っているのはエディタだけ」だから。
`F3` を押しても VM パネルは同じウサギを映し続け、HUD は同じ行に `▸` を付け続ける。
戻るのは `Tab`・クリック・種のボタンで、これは**最初に生き物を選ぶのと同じ仕草**なので、
「F3 をもう一度押す」というトグルは足さなかった（何に戻るのかを言えないトグルになる）。

### 14.1 `F3` は早期 return より前で読む

`choose_watched` は生き物が 1 匹も居なければ即 return していた（`window.rs:101`）。
`F3` をその後ろに置くと、**箱庭が空になった瞬間に規則が読めなくなる**——
規則を読みたい場面として一番ありそうなのは、まさに全滅した直後である。
なのでキーの読みと `editor.picked` の処理を early return の前に出した。順番の変更はそれだけ。

## 15. `Brains` に 3 枠目——「種」でないものを種の入れ物に入れる

計画書の指示は「`Brains` の 3 枠目」。実物は `applied: [Option<String>; 2]` で、添字は `Species::index()`。
素直に 3 にして `Brains::WORLD = Species::ALL.len()` を定数にした。

別の resource を作る道もあった（`WorldBrain(Option<String>)`）。作らなかったのは、
`Brains` が持っている事実が**「種」についてではなく「ゲームが見せているスクリプト」について**だったから:
適用したテキストを Save まで記憶に持つ、`*` を付ける、Revert でファイルに戻す——どれも
生き物の話ではない。実際 `do_world_actions` は `do_editor_actions` の種の枝を**行ごとに写した**形になり、
違うのは「何体再起動したか」と、ボタンが残す文章だけだった。この一致自体が、
境界が正しい場所にあることの証拠として記録しておく。

## 16. 規則の差し替えは W1 が作った道をそのまま使う

`swap_the_rules`（12 番の判定、`main.rs:3212`）の中に閉包 `wear` があって、
`ScriptTask<World>` を外して新しい `Script` を挿していた。W3 はこれを
`wear_the_rules(commands, entity, handle)` として関数に出し、判定もエディタも同じものを呼ぶ。
**判定が先に道を作っていて、窓がその道に乗った**——順番が逆だったら、
「エディタで差し替えられること」を判定が知らないまま実装することになっていた。

`every` のタイマーが `$world_being` を見て自分を止める仕掛け（§11.3）も、そのまま効く。
エディタで Ctrl+Enter を押したときに古い季節のタイマーが残らないのは、W2 で 1 行入れたおかげで、
W3 では何も足していない。関数の doc コメントにその旨を書いた。

**世界の状態は戻らない。** `@season` は古いスクリプトが作ったオブジェクトの ivar なので、
Ctrl+Enter のたびに雨季から始まる。生き物の `@memory` が新しい行動アルゴリズムに引き継がれないのと
同じ理由で、同じように正しい。逆に**コンポーネントに書いたものは残る**——`Breeding` のクールダウンが
規則より長生きするのは W2 §12 が `Reflect` を選んだ一番の理由で、それが W3 で実際に効く場面がこれ。

## 17. `Watch` が `world_prelude.rb` を生き物のものと見なしていた

`reload_changed`（`window.rs:357`）の条件はこうだった:
...
`"world_prelude.rb".ends_with("prelude.rb")` は **true**。つまり W1 で `world_prelude.rb` を
置いた時点から、**そのファイルを保存すると全ビートルと全ウサギが再起動していた**——
そして本来再起動すべき規則の方は古いまま走り続けていた。W1・W2 では窓を実行していないので
誰も見ていない。W3 が `Watch` を触る段階なので、ここで直した:

* 世界の 2 ファイル（`world.rb` / `world_prelude.rb`）を**先に**名前で拾って、規則を差し替えて `continue`。
* 生き物側の `prelude.rb` の判定を `ends_with` から `file_name() == "prelude.rb"` に変えた。

指示に無い変更なので報告に挙げる。1 行の話だが、「W1 が足したファイルが W1 より前の条件に引っかかっていた」
という形なので、直さずに `world.rb` だけ足すと**保存のたびに両方が再起動する**状態になっていた。

## 18. HUD の 1 行と、`WorldMeter` に足した 1 フィールド

計画書は「HUD に世界 VM の命令数/フレーム」。`WorldMeter` は走行全体の分布（`passes`・`most`・`times`）を
持っているが、**今のフレームの値**を持っていなかった——それは予算を選ぶための道具で、
パネルのための道具ではなかったから。`last_pass` を 1 つ足した。

出すのは `VM … ms` の**下の行**で、同じ行に足さなかった理由はコメントに書いた:
予算は VM ごとで合算されないので、規則を高くつくものに書き換えた人が、
生き物の数字を動かさずにそう言えなければならない。実際の表示は（窓を撮った 1 枚から）
...
## 19. 窓のセルフテストは `day_length` を見る

計画書の指示は「窓ありセルフテストに 1 本（F3 で開いて Ctrl+Enter で規則が変わる。W1 の判定 12 の窓版）」。
**何を見れば「規則が変わった」と言えるか**が、ここでも実装より先の問題だった。

判定 12（headless）は「5 秒間どのメーターも下がらない」を見る。窓で同じことをすると
5 秒の窓と `note_the_rules` の仕掛けが要り、窓の検査全体が 5 秒伸びる。
代わりに選んだのは **`day_length`**:

* 太陽は Rust が描くので、`day_length` は規則が**数値として境界を越える**唯一のもの（`garden.rules`）。
* それを言うのは**起動したばかりの世界スクリプト**だけ。
* だから `Sky::day_length == 30.0` は「この規則が走っている」そのもので、
  窓も閾値も統計も要らない。

手順は `F3` → `editor.text` の `day_length 60.0` を `30.0` に → **Ctrl+Enter を本当に押す** → 0.6 秒後に確認 →
`Revert` → 60 秒に戻ることを確認。11 行増えて、窓の検査は 29 行から **40 行**になった。

### 19.1 キーを本当に押せるかを先に確かめた

既存の検査はエディタを `editor.action = Some(...)` で動かしている（「クリックと同じやり方で」）。
Ctrl+Enter を読んでいるのは `rubevy-arena` の `draw_editor` で、これは
`EguiPrimaryContextPass`——`bevy_egui 0.42` が **`PostUpdate` に置くスケジュール**（`lib.rs:1143`）——で走る。
`window_selftest` は `Update` なので、同じフレームの後ろで `just_pressed` が生きたまま読まれ、
そこで立った `editor.action` を次のフレームの `do_editor_actions` が取る。0.6 秒待つので間に合う。
**確かめてから書いた**ので、キーを押す形にできた。

### 19.2 `window_selftest` の順序を 1 つ足した

`F3` を読むのは `choose_watched` で、`window_selftest` は `.before(window::inspect_keys)` としか
言っていなかった。`choose_watched` は chain の先頭なので、**押す前に読まれるフレームがありうる**——
`just_pressed` は次のフレームの `PreUpdate` で消えるので、その回のキーは誰も見ないまま消える。
`.before(window::choose_watched)` を足した。F2 が `inspect_keys` に対して同じ理由で
順序を持っていたのと同じ話で、キーが 1 つ増えたら読む系も 1 つ増える。

## 20. ブラウザ側は**何もしなくてよかった**——確かめた

計画書は「`build.rs` が `ruby/world.rb` / `world_prelude.rb` を拾うこと、`localStorage` の鍵」。
どちらも**既にそうなっている**ことを確かめただけで、行は 1 つも足していない:

* `build.rs` は `ruby/` を**再帰で舐めて全 `.rb`** を表にする（`collect`）。生成物を見た:

  ```
  ("ruby/creatures/beetle.rb", …) ("ruby/creatures/rabbit.rb", …) ("ruby/prelude.rb", …)
  ("ruby/world.rb", …) ("ruby/world_prelude.rb", …)
  ```

  wasm 側の `target/wasm32-unknown-unknown/web/build/garden-*/out/ruby_files.rs` も同じ 5 行で、
  ビルドした `game_bg.wasm` に `ruby/world.rb` と `world_prelude.rb` の文字列が入っている。
* 鍵は `platform::write` が `format!("{STORE}{path}")` を作るだけで、`STORE` は `"garden:"`、
  パスはブラウザでは `ruby_dir() = "ruby"` から `ruby/world.rb`。つまり **`garden:ruby/world.rb`**。
  エディタの Save は種と同じ `platform::write` を通るので、ここも行が要らない。

「何もしなくてよい」と分かるまでに読んだのは `build.rs` 全部と `platform.rs` の wasm 側で、
**読まずに足していたら二重の仕掛けになっていた**ところ。`web/build.sh garden` は通る
（`wasm-opt` がこの機械に無いので縮んでいない旨の警告は出る。環境の話で、変更の話ではない）。

## 21. 説明とフォント

`guide_text.rs` は**行 1 つと段落 1 つだけ**触った（著者が main 側で同じファイルを開いているため、
マージの衝突を最小にする指示）。既存の文言は 1 文字も変えていない。

段落は「世界の規則も Ruby です。F3 を押すと world.rb がエディタに開きます。…書き換えて Ctrl+Enter を押すと、
箱庭を止めないまま新しい規則で動き続けます。」、キーの行は `F3 / the rules of the world (world.rb) / 世界の規則(world.rb)`。

フォントは切り直す前に `docs/worklog/2026-09-17-garden-guide-rewrite.md` の 1 行で調べた。
**足りなかったのは 5 文字: `件条減腹開`。**（`python3 -m venv` に fonttools を入れて cmap と突き合わせる。
リポジトリには入れていない。）`tools/subset-font.sh` で切り直して 0 になった:
**64,944 → 66,372 バイト、グリフ 329 → 334**。日本語のガイドを開いた窓の絵で、豆腐が無いことを目で確かめた。

`docs/web.md` の「63 KB is the font」は 64.8 KB になったが、**更新していない**——
あの表は G6 でモジュール全体のサイズを測ったもので、フォントだけ書き換えると
表の他の列と整合しなくなる。測り直すのは W3 の範囲ではないので、ここに書いておく。

## 22. 予算の測り直し——W1 の根拠は W2 の規則には効かない

これが W3 で一番時間を使ったところ。

W1 は予算 45,000 を**こう決めていた**（§4.3）: 1 パスの命令数を人口の 1 次式に当てはめ
（`262 + 159×草 + 280×生き物`、残差 0.9%）、**上限の 2 倍の箱庭**（180 株・48 匹）を代入して 42,300、
それを丸めた。W2 は「観測倍率 ×1.46 を掛けると上限で約 29,000」と**外挿**を書いて、
測り直しを著者判断に残していた。W3 の指示は、その外挿を測定にすること。

### 22.1 まず W1 と同じ手順を踏んで、法則が**壊れている**ことが分かった

`world_clock_end` の後ろに使い捨ての系を 1 本足して、毎フレーム
`(1 パスの命令数, 草, 生き物, tick の ms)` を出した（W1 と同じ道具）。
普通の箱庭（55 株・6+4 匹）90 秒 ×2 = 10,747 フレームを最小二乗で:

```
命令数 ≈ 7,070 + 95.8 × 草 + 145.6 × 生き物      （残差 rms 714 = 中央値の 5.2%）
```

W1 の残差は 0.9%、切片は 262 だった。**切片が 7,070 に膨らみ、生き物 1 匹あたりの係数が半分になった**——
当てはまりが悪いときの典型的な形で、モデルが間違っている。
実際、この式に上限（90 株・24 匹）を入れると 19,186 で、**実測の中央 24,670 より 22% 低い**。

理由は W2 が足したもののうち**つがい探し**が人口の 1 次式でないから。
「腹が 75 以上の生き物どうし」の探索なので、人口が増えると候補の対の数がそれ以上に増える。
1 次式で外挿できるのは、パスの中身が「各エンティティを 1 回ずつ舐める」だけのときで、W1 はそうだった。

**捨てた案**: 2 次の項を足して当てはめ直す。やめた理由は、そこまでするなら
**上限で直接測ればよい**から——上限は外挿ではなく、規則自身が決めている場所（`plant_cap` 90、`POP_MAX` 24）で、
そこに座らせることができる。

### 22.2 上限に本当に座らせる

W1 の手順（`PLANTS_AT_START` 90、`BEETLES`/`RABBITS` 16/8）で走らせると、**草が上限に届かない**:
24 匹が食べるので 42〜79 株で落ち着く。W1 の「上限で測った」は、正確には
「生き物が上限、草はその下」だった（W1 §4.2 の 45〜82 株がそれ）。

なので `PLANTS_AT_START` を **130** にした。規則は 90 を超えたら芽を出さないだけで、
最初から多い分を減らしはしないので、**草が 90 を通過する間、生き物は 24 匹**という
フレームが取れる。60 秒 ×3、そのうち**草 89〜91 株・生き物 24 匹の 392 フレーム**:

| | 中央 | 99 パーセンタイル | 最大 |
|---|---|---|---|
| 1 パスの命令数 | 24,670 | 25,782 | **25,837** |
| 世界 VM の tick | 2.65 ms | 4.60 ms | 4.86 ms |

参考: W1 の同じ場所は最大 20,128。W2 の規則は上限で **×1.28**。
W2 が外挿で書いた「約 29,000」は 12% 高かった（倍率を最大どうしで掛けたため）。

### 22.3 数は動かさない。根拠を差し替える

**45,000 のまま。** 理由:

* 上限の実測最大 25,837 に対して **1.74 倍**。規則を今の 1.5 倍の仕事に書き換えても届かない。
  「暴走した規則」（`sleep` の無いループ）は倍率がいくつでも 1 フレームで止まる。
* 生き物 VM の 200,000 の 1/4 弱という関係も変わらない。
* 動かすとしたら新しい根拠が要るが、正直に出せる根拠は 25,837 と、時間側の 8 ms しかない。
  45,000 はその**間**にある。

**変わったのは `frame_time` との関係**で、これは書き直した。W1 は
「45,000 命令 ≈ 8.6 ms で、2 つの数がほぼ同じ限界を指す」と言っていた。W2 の規則の実測は
1 ms あたり約 9,300 命令（上限で 中央 24,670 命令 / 中央 2.655 ms）なので、45,000 は**約 4.8 ms**。
つまり**先に噛むのは命令数の方**になった。これは悪い変化ではない:
命令数は規則についての事実で、数倍遅いブラウザでも同じ数だが、ミリ秒は機械についての事実だから、
先に噛むのが命令数である方が、PC とブラウザで同じところで止まる。
`install_world_answers` のコメントを全部書き直して、この経緯（W1 の法則をなぜ取り下げたか）も残した。

計測系は数を決めたあと消した。`PLANTS_AT_START` / `BEETLES` / `RABBITS` も戻してある。
...
|---|---|
| `cargo build --release -p garden -p sabibots` | 通る |
| `cargo clippy -p garden` | **13 件。W1・W2 と同じ数**（新しい警告は無い。`do_world_actions` と `reload_changed` には `#[allow(clippy::too_many_arguments)]` を付けた——既存の 4 つと同じ扱い） |
| garden `--headless 90` ×5 | **13 判定すべて ok、5 回とも**（§23.1） |
| sabibots `--headless 25` ×1 | **64 件すべて ok、FAIL 0**。無変更 |
| **窓ありビルドの実行**（docker + lavapipe） | **40 行すべて ok**（うち W3 の 11 行）。`--shot` 2 枚（HUD の新しい行、日本語ガイド） |
| `web/build.sh garden` | **通る**（`wasm-opt` がこの機械に無いので縮んでいない旨の警告） |
| セーブの往復（`--headless 25 --save` → `--headless 0 --load --save`） | **バイト一致** |
| 走行中のロード（`GARDEN_RELOAD_AT=20`、F9 の道） | **バイト一致** |

窓は WSL に GPU ドライバが無い（`/dev/dri` が無い）ので `docs/wsl-gpu.md` の道——
`rubevy-games-build` イメージ + lavapipe + WSLg——で走らせた。`docker info` は通ったので
Docker Desktop は起動していない。target は `rubevy-games-target-world` という別のボリュームにして、
main 側のビルドを上書きしないようにした。

### 23.1 5 回の 13 判定

| 判定 | 1 | 2 | 3 | 4 | 5 |
|---|---|---|---|---|---|
| 4 nothing walked through anything | 0.975 | 0.976 | 0.978 | 0.974 | 0.976 |
| 5 …reached it | 1.78 | 1.79 | 1.79 | 1.79 | 1.79 |
| 7 asleep a second after night | ok | ok | ok | ok | ok |
| （他 10 件） | ok | ok | ok | ok | ok |

W2 で 10 回中 1 回ずつ落ちた 5 番・7 番は、この 5 回では落ちていない。
**5 回では「直った」とは言えない**（元々 10〜26 回に 1 回の話）ので、言えるのは「悪くなっていない」まで。
W3 は規則も判定も 1 行も変えていないので、そもそも率が動く理由が無い。

総計行（5 回、普通の箱庭）:

```
hud: the world's rules — 5373〜5374 passes of `each_frame`,
     11,898〜14,663 instructions at the median and 14,514〜17,571 at the most (of 45000);
     the world's tick 1.91〜2.11 ms at the median, 3.97〜4.14 at the 99th, 5.00〜6.58 at the most / 8.0 ms
```

## 24. 置いていくもの・気づいたこと

1. **世界の VM パネルは無い。** `F3` はエディタだけを切り替え、`F2` のパネルは生き物のまま。
   `VmInspector` は `ScriptWorld` を名前で取る（`rubevy-arena`）ので、2 本目を映すには
   パネル側に VM を渡す口が要る。計画書 §4 の W3 に無いので手を付けていない。**著者判断待ち**。
2. **世界のエディタに行の帯と熱が無い。** どちらも `watch_minds` が生き物について測っているもので、
   世界のスクリプトには `Mind` が無い。1 フレームに 1 回上から下まで走るパスに
   「よく戻ってくる行」は無いので、足しても意味が薄い。これも計画書の範囲外。
3. **コンパイルエラーの行番号には prelude 分のずれが乗る**（W1 §8 で「W3 のエディタの仕事」と書いたもの）。
   実際に読んだら、**生き物側も同じずれが乗っている**（`compile_source` も 1 本にしてから渡す）ので、
   世界だけが特別に悪いわけではなかった。エディタが出すのは
   `not applied: world.rb would not compile (see the log)` で、行番号はログの方にある——
   種と完全に同じ振る舞い。直すなら両方いっしょに直す話で、W3 の範囲ではない。
4. **`editor.noun` は世界のとき `"world"`。** ボタンの hover は
   「run this in the world shown」「keep it: worlds on the file start with it from now on」になる。
   後者は日本語にすると変な文で、英語でも少し変。`rubevy-arena` の hover を
   2 ゲーム 3 単位に耐える形にする話なので、W3 では触っていない。
5. **`docs/web.md` のフォントのサイズ**（§21）。
