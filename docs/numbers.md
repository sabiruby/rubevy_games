...
`a_jam_keeps_the_gap_even_when_the_gap_is_not_an_exact_number`）。f32 の 1/3 は切り上げなので
隙間 3 つがタイル 1 枚より長くなり、4 個目が継ぎ目で断られる。**不変条件（隣と spacing 以上、
1 個も失わない）はどの値でも保たれている**ので直していない。詳しくは
`worklog/2026-09-21-factory-F1.md` §5.2。

**0 以下の設定は断る。** `Rules` の 6 件と `camera_half_height` は、読み込み時に
`main.rs` の `positive` / `counted` が 0 以下（と、個数なら 1 未満に丸まるもの）を log に出して
既定値へ戻す。**算術の側に下限を置かない**ためで、`1.0 / items_per_tile.max(0.001)` の
0.001 のような「割り算を止めるためだけの数」は出どころを言えないから消した（F1 の終わりの
見直しで 3 か所）。
...
0 以下を読み込み時に断る形（§9.3 の最後）に移した。`chest_capacity` / `ore_per_tile` の
`.max(1.0)` も同じ道へ移した（個数は 1 以上、という数え方の話であって閾値ではない）。
