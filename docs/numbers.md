...
| `factory/src/main.rs` | `TILESET_LAYERS` | 133 | シートの層の数。**6 の倍数にするとブラウザで何も描かれない**（配列テクスチャがキューブマップ配列として bind される）。判定がこの数を読み返す | 導出: Kenney の 132 + 6 の倍数から外すための空白 1。`tools/factory-tileset.py` が計算して書く。根拠は `wgpu-hal-29.0.4/src/gles/mod.rs:458`、実測は 2026-09-21 のブラウザ走行 |
