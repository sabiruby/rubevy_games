...
移った**。F1 が設定に置いていた 6 件のうち 4 件が F2 で Ruby へ、**残る 2 件のうち鉱石の量も
F2a で Ruby へ**（`ore :patch, per_tile:`）。設定に残ったのは **畑の半径 1 件だけ**で、それは
「遊ぶ前に世界を組む数」というより、`map_tiles` の下限がそこから導かれ、その下限が
`main()` の中、**まだ VM が無い所**で要るからである（§9.2 の最後）。
...
| `ore_patch_radius` | 3.0 | **導出**: 畑は地図の 4 つの象限の中心（`tiles/4`, `3×tiles/4`）に置く。半径 3 なら 15 タイルの地図でも畑が縁（タイル 0 の壁）に掛からない（§9.4 の下限）。**F2a のあと、設定に残る唯一の遊びに近い数** |
...
**設定に残ったのは `ore_patch_radius` だけで、理由は 1 つだけになった。** F2 は 2 件とも残し、
理由を「どちらも `Ore::laid_out` が 1 回だけ読む世界の組み立ての数だから」と書いたが、それは
**2 件をひとまとめに見た言い方**で、片方（1 タイルに何個埋まっているか）には当たっていなかった。
当たっているのは**畑の半径だけ**である: `map_tiles` の下限は `ore_patch_radius` から導かれ、
その下限は `main()` の中、`Map` を作る前 — **VM がまだ無く、`data.rb` を読む手段が無い所**で要る。
**`ore_per_tile` にはその事情が無い**（`Ore::laid_out` は `Startup` の data stage の後に走る）ので、
F2a で `data.rb` へ移した。**これで「箱 1 杯ぶん」という導出がファイルをまたがなくなった** —
F2 の時点では `chest :crate, capacity: 60` が Ruby で `ore_per_tile=60` が設定にあり、片方を
動かしても片方は動かない形になっていた（計画書 §7 の 09-21 F2 の行）。線は
「**VM の前に要るか**」で引き直した（F2a の worklog §4）。
...
| `ore :patch` | `per_tile` | 60 | **導出**: 鉱石 1 タイルで**箱 1 杯**（すぐ上の `capacity` と同じ数で、同じ「1 分」である）。**F2a で `factory.settings.txt` から移した**ので、この導出は 1 つのファイルの中で閉じている。半分を切ると絵が「残り少ない」タイルに変わる。名前 `:patch` はラベル（`:conveyor` / `:crate` と同じで、どこからも引かれない）。地面から出てくるのは `miner` の `digs:` |
...
Factory は **37 件**（不変量 (a) 6 行 19 個、動かす側 (c) **8**、**遊びの数 (b) 12**（`data.rb`）、
...
`mine_seconds`・`chest_capacity`）。**F2a でもう 1 件**（`ore_per_tile` → `ore :patch, per_tile:`）。
