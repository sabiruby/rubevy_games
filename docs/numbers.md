...
| `factory/src/main.rs` | `TILESET_LAYERS` | 136 | シートの層の数。**6 の倍数にするとブラウザで何も描かれない**（配列テクスチャがキューブマップ配列として bind される）。判定がこの数を読み返す | 導出: Kenney の 132 + 自作のベルト 4（著者が選んだ (ii) の直線と曲がり、各 2 コマ）。136 は 6 の倍数ではないので空白は要らない。`tools/factory-tileset.py` が計算して書く。根拠は `wgpu-hal-29.0.4/src/gles/mod.rs:458`、実測は 2026-09-21 のブラウザ走行 |
...
見本（`factory/src/belt_sample.rs`）が持っていた 2 つ — `TILES` 16 と `half_height()` 96 — は、
著者が 2026-09-21 に (ii)（真上からのベルト）を選んだ時点で、見本のファイルごと消えた。
一覧の件数には最初から入れていない。

### 9.5 素材の容量の上限

**64 KB、1 枚のシート + 使ったパックごとのライセンス原文。** 出どころ: シートの実測が
**1 タイルあたり 67 B**（F0 の 145 タイルで 9,662 B、2026-09-21。(i) を落とした 136 タイルでも 66.6 B）
なので、512 タイルのシートでも約 34 KB。
64 KB はその倍で、既存 2 本の小さい方（sabibots 234,202 B）の 27%。
今の実績は 9,632 B（2 ファイル、上限の 15%）。詳しくは `docs/factory.md`。
