...
| VM の予算 | rubevy の既定 200,000（**S10 でも動かしていない。著者判断待ち**） | — | ● | `script_budget` | (c) | **値の出どころは今も不明**（rubevy の既定のまま。rubevy 自身が「出どころ不明」と書いている）。**S10 が測った**（2026-09-21、上限の試合 = 4 チーム × 9 台 = 36 台、16 走行・26,608 フレーム）: 1 フレームの命令数は中央値 3.6k・99 パーセンタイル 45k・**最悪 67,749**、tick は中央値 0.22 ms・最悪 8.83 ms、**持ち越しは全フレームで 0**、予算に届いたフレームは 0。同梱の試合（4 台）は最悪 2,730 = 既定の **1.4%**。**案は `1.74 × 65,704 → 114,000`（`handler_frames` は 7 → 5）で、著者判断待ち**——測定と案と「何を得て何を失うか」は `docs/worklog/2026-09-21-s10.md` §2。測ったのは「今の値で足りている」ことであって、200,000 がどこから来たかではない |
...
200,000（rubevy の既定）のままだが、**S10 が上限の試合に座って測った**（§4.5 の行）——
既定値は動かしておらず、案（114,000）と導出は `docs/worklog/2026-09-21-s10.md` §2 にあって
**著者判断待ち**である。箱庭の測った分布は
...
## 9. Factory（F0・F1・F2。2026-09-21）

3 本目の crate `factory/` が持っている数の全部。節の番号が 8 の後ろなのは、
§2〜§5 の番号が他の文書から参照されているため（renumber しない）。
過程は `docs/worklog/2026-09-21-factory-F0.md`・`…-F1.md`・`…-F2.md`。
...
移った**。F1 が設定に置いていた 6 件のうち **4 件が Ruby へ**、**2 件（鉱石の量と畑の半径）は
設定に残った** — 後者は「遊ぶ前に世界を組む数」で、`map_tiles` の下限がそこから導かれ、
その下限は `main()` の中、**まだ VM が無い所**で要るから（§9.3 の頭）。
...
| `factory/src/main.rs` | `TILESET_LAYERS` | **142**（F1 は 139） | シートの層の数。**6 の倍数にするとブラウザで何も描かれない**（配列テクスチャがキューブマップ配列として bind される）。判定がこの数を読み返す | 導出: Kenney の 132 + ベルト 4 + 鉱石 2 + 組立機 4 = 142。142 % 6 = 4 なので `pad_to_a_safe_count` は空白を足さない（F1 の 138 では足していた）。根拠は `wgpu-hal-29.0.4/src/gles/mod.rs:458`、実測は 2026-09-21 のブラウザ走行 |
| `factory/src/draw.rs` | `ITEM_PX` | 8 | アイテムの絵の大きさ。シートを切る所 | 導出: 16 px のタイルに 2 個が触れずに載る大きさ。`data.rb` の `items_per_tile` の既定はこの数から出ている（逆ではない） |
| `factory/src/draw.rs` | `ITEM_ICONS` | 3 | `assets/items/items.png` の絵の枚数。`data.rb` の `icon:` はここへの添字 | 引用: `tools/factory-items.py` の `ICONS`。単体テストが `data.rb` の `icon:` が全部この範囲にあることを見る |
...
| 鍵 | 既定値 | 出どころ |
|---|---|---|
| `map_tiles` | 32 | **F0 の仮の値のまま。F1 でも F2 でも決められなかった**（§9.6）。下限は導出（§9.4） |
...
| `headless_seconds` | **20**（F1 は 10） | **導出（F2 で取り直した）**: 判定は 2 つの小さな工場を順に建てる。F1 の（採掘機・ベルト 3 枚・箱）は `mine_seconds + 4 ÷ belt_tiles_per_second` = 3.0 秒、F2 の（ベルト→機械→ベルト→箱）は `2 ÷ belt + time ÷ speed + 1 ÷ belt` = かまど 3.5 秒。合わせて 6.5 秒で、20 はその 3 倍強。**上限であって待ち時間ではない** |
| `shot_file` / `shot_seconds` | `shot.png` / **8.0**（F1 は 3.5） | **導出（F2 で取り直した）**: 絵に写したいのは「工場が動いているところ」で、判定の 2 つ目の工場が終わるのが 6.5 秒。8 はその少し後 |
| `ore_per_tile` | 60 | **導出**: 鉱石 1 タイルで**箱 1 杯**（`data.rb` の既定の `chest_capacity` と同じ 60）。data.rb の箱を変えてもこの数は動かない — 動かしたければ設定で動かす。半分を切ると絵が「残り少ない」タイルに変わる |
| `ore_patch_radius` | 3.0 | **導出**: 畑は地図の 4 つの象限の中心（`tiles/4`, `3×tiles/4`）に置く。半径 3 なら 15 タイルの地図でも畑が縁（タイル 0 の壁）に掛からない（§9.4 の下限） |
| `stress_items` | 0（しない） | 計測の仕掛け。`--stress N` / `FACTORY_STRESS` / `?stress=N` でも同じ。**地図の大きさは走行が自分で決める**（√(N ÷ `items_per_tile`)）。F2 でその計算は `Startup`（data stage の後）へ移った — `items_per_tile` はもう `main()` では読めないから |

`camera_*`（ズームの刻み、ドラッグの比、クリックの遊び）は共有 crate の 7 鍵をそのまま使う（§1.2）。

**`ore_*` の 2 件が設定に残った理由。** どちらも `Ore::laid_out` が **1 回だけ**読む「世界の組み立て」の数で、
工場の 1 歩はどちらも読まない。そして `map_tiles` の下限は `ore_patch_radius` から導かれ、
その下限は `main()` の中、`Map` を作る前 — **VM がまだ無く、`data.rb` を読む手段が無い所**で要る。
「遊びの数は Ruby」の線はそこで引いた（F2 の worklog §4）。
...
| | `items_per_tile` | 2.0 | **導出**: アイテムの絵が 8 px、タイルが 16 px なので、触れずに並ぶのは 2 個。**満杯のベルトは毎秒 4 個運ぶ**（2.0 × 2.0）で、下の全部がこの 4 に対する割合である |
| `miner :drill` | `seconds_per_item` | 1.0 | **導出**: ベルトの **1/4**（毎秒 1 個）。4 台で 1 本が埋まる |
| | `digs` | `:iron_ore` | 参照。地面から出てくるアイテムの名前 |
...
**「隙間 N 個の列には N+1 個」が成り立つのは `1 ÷ items_per_tile` が f32 で正確なときだけ**
（= 2 の冪。既定の 2 はそう）。F1 の終わりに 1・2・3・4・5・7・8 を実際に走らせて数えたら、
**3 だけが 3 個**で、残りは全部 N+1 個だった（`belts.rs` の
`a_jam_keeps_the_gap_even_when_the_gap_is_not_an_exact_number`）。**不変条件（隣と spacing 以上、
1 個も失わない）はどの値でも保たれている**ので直していない。詳しくは
`worklog/2026-09-21-factory-F1.md` §5.2。
...
9,612 B = 67.7 B）なので、512 タイルのシートでも約 35 KB。
64 KB はその倍で、既存 2 本の小さい方（sabibots 234,202 B）の 27%。
今の実績は **11,064 B / 4 ファイル**（シート 9,612 + アイテムの絵 317 + ライセンス 2 通 1,135）で、
...
満杯のベルト 1 本ぶんの製品は**その 4 倍 = 28 台**である。上の 2 つのどちらかが「鎖を何本置けるか」
を言えば、地図の大きさはそこから出る。**もっともらしい数を今作らない**（CLAUDE.md）。

### 9.7 件数

Factory は **37 件**（不変量 (a) 6 行 19 個、動かす側 (c) 9、**遊びの数 (b) 11**（`data.rb`）、
判定と計測の器 (d) 5、導出で設定でないもの 1、容量 1）。
出どころは 測った 1 / 導出 26 / 引用 5 / **不明 1**（窓の大きさ）/
...
`mine_seconds`・`chest_capacity`）。`ore_per_tile` と `ore_patch_radius` は残った（§9.2 の最後）。
