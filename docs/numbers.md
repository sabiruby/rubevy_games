...
**F6 がその「1 度」を、そのまま試せる形にした**（2026-09-22）:
**[`verification/factory-on-a-real-gpu.md`](verification/factory-on-a-real-gpu.md)**（5 行）。
公開ページの `?arms=1` が **`data.rb` の言う地図のまま**測り、
`stress:` の行が **1 行で**「地図の大きさ・アイテム数・フレーム ms の p50/p95・step µs・
GPU のアダプタ名」を言う（F6 が地図とアダプタ名をその行に足した）。
**URL から地図の大きさを渡す口は作らなかった** — 地図は F3a から `data.rb` の宣言で、
ページでもエディタで書き換えて Apply できる。URL に 2 つ目の言い方を作ると綴りが 2 か所になる。
見るのは 1 つだけ、**`frame ms` が 16.7（vsync）を離れるのはどの地図からか**。
...
#### 9.8a F6 で取り直した（2026-09-22）— **既定値は動かさない**

F3 が 39,000 を決めたあと、**F4 の control stage と F5 の `Held` への乗り換え**（prelude が 43 行
短くなった）が入った。計画 §4 の F6 の行が言うとおり、上限の工場で取り直した。
器は同じ `--arms N`、release、窓なし、**交互に 2 巡 + 向きを入れ替えてもう 2 巡**（計 8 走行）、
各走行の 3 回の報告のうち**最初の 1 回は捨てる**。機械は `vmstat` で 99% idle。

| 腕 | 命令 p50 | 命令 p95 | tick µs p50（最小〜最大） | tick µs p95 | 持ち越し p95 | 落とした | step µs p50 |
|---|---|---|---|---|---|---|---|
| **340**（既定の地図の上限 = 鎖 34 本） | 1,363〜1,579 | **2,227** | 139〜196 | 258〜453 | **0** | **0** | 17〜24 |
| **3,000**（どの地図も持てない数） | 10,219〜10,435 | **14,971**（1 走行だけ 17,347） | 1,157〜1,450 | 2,527〜3,410 | **0** | **0** | 108〜126 |

**命令数は決定的**（同じ腕数なら走行をまたいで 1 命令まで同じ。p50 が 2 つあるのは
`inserter_stagger` の `srand(me.to_i)` が腕の並びで散らす位相の違い）で、µs だけがばらつく。

**予算を導き直すと 36,000〜39,000 になり、39,000 と区別がつかない。** rubevy の 3 歩をそのまま:

1. **速さ**: 3,000 台（忙しいほう）で 10,435 命令 ÷ 1,157 µs = **毎 ms 9,019 命令**（同じ巡の最小の
   tick で取る。中央値の tick 1,298 µs で取れば 8,039、340 台では 9,806）。F3 は 9,800 だった。
2. **取り分**: 変わらず 1 フレームの 1/4 = 4 ms。
3. **買える命令数**: 4 × 9,019 = **36,000**。

差は 8% で、**tick µs 自身の走行間のばらつき（1,157〜1,450 = 25%）より小さい**。
つまり同じ導出が今日出す幅の中に 39,000 が入っている。**自分の測定のぶれの内側で既定値を
動かすのは、数を動かしたことにならない**ので動かさない。
実害のほうも 0 である: **持ち越しも `dropped` も 8 走行 24 報告すべてで 0**、
既定の地図の上限（340 台）での最悪のフレームは 2,227 = 予算の **5.7%**、
3,000 台でも 17,347 = **44%** で、どちらも噛んでいない。

**F5 の窓（HUD・VM パネル・エディタ）はこの数に入っていない** — 器はヘッドレスで、
