...
## 9. Factory（F0。2026-09-21）

3 本目の crate `factory/` が F0 で持っている数の全部。節の番号が 8 の後ろなのは、
§2〜§5 の番号が他の文書から参照されているため（renumber しない）。
過程は `docs/worklog/2026-09-21-factory-F0.md`。

**方針**: 「遊びの数は Ruby、動かす側は `factory.settings.txt`、`const` は不変量だけ」
（計画書 `plans/factory-plan.md` §2）。F0 には遊びの数がまだ 1 つも無いので、全部 (a) か (c) である。
...
| `factory/src/main.rs` | `TILESET_LAYERS` | 145 | シートの層の数。**6 の倍数にするとブラウザで何も描かれない**（配列テクスチャがキューブマップ配列として bind される）。判定がこの数を読み返す | 導出: Kenney の 132 + 自作のベルト 12 + 6 の倍数から外すための空白 1。`tools/factory-tileset.py` が計算して書く。根拠は `wgpu-hal-29.0.4/src/gles/mod.rs:458`、実測は 2026-09-21 のブラウザ走行 |
| `factory/src/main.rs` | `FLOOR_PLATES` / `GROUND` | `[0,1,2]` / `3` | シートのどのタイルか。F1 が床ごと捨てる | 引用: シートを 2026-09-21 に読んだ（`docs/factory.md` の表） |

### 9.2 動かす側 (c) — `factory.settings.txt`

| 鍵 | 既定値 | 出どころ |
|---|---|---|
| `map_tiles` | 32 | **導出（F0 の仮の値）**: 窓が縦に見せる 16 タイルの 2 倍。「世界が窓に入りきらない」ことがパン・ズームを確かめる条件だから。F1 で 3.7 の「上限の機械が無理なく置ける広さ」から決め直す |
| `camera_half_height` | 128（= 8 タイル） | **導出（F0 の仮の値）**: 縦 16 タイル。既定の 900 px の窓で素材の 1 px が約 3.5 画面 px になり、「床が見える」。F1 で `map_tiles` と一緒に決め直す |
| `window_width` / `window_height` | 1600 / 900 | **不明**。既存 2 本と同じ値にしただけで、1600×900 という数の出どころはどちらにも無い（`sabibots/src/main.rs`「Source unknown」） |
| `headless_seconds` | 10 | **測った（2026-09-21）**: F0 の走行は全部で 0.3 秒で終わる（判定は条件で待ち、済んだら終わる）。10 秒はその 30 倍で、上限であって待ち時間ではない |
| `shot_file` / `shot_seconds` | `shot.png` / 2 | **測った（2026-09-21）**: 床はタイルセットが届いた次のフレームに出る（コンテナで 0.4 秒）。2 秒はその余裕 |
...
### 9.4 F0a の見本だけの数（`factory/src/belt_sample.rs`。**F1 が消す**）

| 名前 | 値 | 出どころ |
|---|---|---|
| `belt_sample::TILES` | 16 | 導出: 見本の絵が 16 文字幅 |
| `belt_sample::half_height()` | 96（= 6 タイル） | 導出: 見本の絵は 11 行で、上下 1 タイルの余白を足して 12 タイルが窓に入る高さ |

見本は著者が (i) か (ii) を選ぶための絵で、選んだら片方と `belt_sample.rs` ごと消える。
一覧の件数には入れない。

### 9.5 素材の容量の上限

**64 KB、1 枚のシート + 使ったパックごとのライセンス原文。** 出どころ: シートの実測が
**1 タイルあたり 67 B**（145 タイルで 9,662 B、2026-09-21）なので、512 タイルのシートでも約 34 KB。
64 KB はその倍で、既存 2 本の小さい方（sabibots 234,202 B）の 27%。
今の実績は 10,231 B（2 ファイル、上限の 16%）。詳しくは `docs/factory.md`。

### 9.6 件数

Factory は 9 件（不変量 3、設定 5、判定 1）。出どころは 測った 3 / 導出 3（うち 2 つは「F0 の仮の値」）/ 引用 2 / **不明 1**（窓の大きさ）。
