...
robot "Scout" do
  ...
  # *lower* priority than the brain, so within a frame the handler asks last and the game answers
  ...
  def run
    @side = rand < 0.5 ? 1 : -1
    loop do
      # while a handler has the wheel the brain keeps its hands off it
      if @swerve
        sleep 0.05
        next
      end
